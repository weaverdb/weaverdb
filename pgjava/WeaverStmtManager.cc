...
WeaverStmtManager::WeaverStmtManager( const char* name, const char * paslong, const char* connect )
{
	...
	stackSize = 0;
        clean = false;
/*  serious problem allocating connection  */	
...
WeaverStmtManager::~WeaverStmtManager( void )
{
	holdingArea = 0;
	if ( theConn != NULL ) {
		PGDestroyConnection(theConn);
...
WeaverStmtManager::WeaverStmtManager( WeaverStmtManager* parent ) 
{
...
WeaverStmtManager* WeaverStmtManager::CreateSubConnection( void ) 
{

    WeaverStmtManager * setup = new WeaverStmtManager(this);
    return setup;
}

short WeaverStmtManager::IsValid( void )
{
...
void WeaverStmtManager::Clean(JNIEnv* env)
{
    if ( clean ) {
        Init(env);
        clean = false;
...
void WeaverStmtManager::Init(JNIEnv* env)
{
	short x;
	bindFlag = 0;
        /*
...
void WeaverStmtManager::Clean( void )
{
    if ( clean ) {
        Init();
        clean = false;
...
void WeaverStmtManager::Init( void )
{
	short x;
	bindFlag = 0;
...
short WeaverStmtManager::Begin( void ) 
{
	int err = 0;
	statementLength = 0;
	int counter = 0;
...
short WeaverStmtManager::Fetch( void ) 
{
/*	ClearData();		*/
	PGFetch(theConn);
	return CheckLowerError();
...
short WeaverStmtManager::Cancel( void ) 
{
        if ( !IsValid() ) return -1;
...
short WeaverStmtManager::Prepare( void ) 
{
        if ( !IsValid() ) return -1;
...
short WeaverStmtManager::BeginProcedure( void ) 
{
        if ( !IsValid() ) return -1;
...
short WeaverStmtManager::EndProcedure( void ) 
{
        if ( !IsValid() ) return -1;
...
short WeaverStmtManager::Rollback( void ) 
{
        if ( !IsValid() ) return -1;
	PGRollback(theConn);
	transactionId = 0;
...
short WeaverStmtManager::Exec( void ) 
{
    int x=0;
        if ( !IsValid() ) return -1;
   ...
   *  a value  */              
        for (x=0;x<20;x++)
	{
		if (bindLog[x].binder[0] == '\0') break;
                if ( bindLog[x].indicator < 0 ) {
                    char        msg[255];
                    snprintf(msg,255,"bound variable %s not set",bindLog[x].binder);
                    return DelegateError("EXEC",msg,849);
                }
        }
...
short WeaverStmtManager::Commit( void )
{
        if ( !IsValid() ) return -1;
...
short WeaverStmtManager::UserLock(const char* grouptolock,uint32_t val,char lock)
{
        if ( !IsValid() ) return -1;
...
int  WeaverStmtManager::GetErrorCode( void )
{
        if ( errorlevel != 1 && !IsValid() ) return -1;
	return (errorlevel == 1) ? PGGetErrorCode(theConn) : errordelegate.rc;
}

const char*  WeaverStmtManager::GetErrorText( void ) 
{
        if ( errorlevel != 1 && !IsValid() ) return "connection not valid";
	return (errorlevel == 1) ? PGGetErrorText(theConn) : errordelegate.text;
}

const char* WeaverStmtManager::GetErrorState( void )
{
        if ( errorlevel != 1 && !IsValid() ) return "INVALID";
	return (errorlevel == 1) ? PGGetErrorState(theConn) : errordelegate.state;
}


short WeaverStmtManager::ParseStatement( const char* thePass, long passLen)
{	
	statementLength = 0;
...
short WeaverStmtManager::AddBind(const char * vari, long theType)
{
	short	x;
...
short WeaverStmtManager::SetLink(const char * vari,long theType,void* theData,int theLength)
{
	short   x;
	long 	storeLength = 0;
	int     pipe_pos;
...
#ifdef PG_JAVA
short WeaverStmtManager::OutputLinker(jobject theRef, const char* slotname,const char* signature,
				const char* className, long theType, long index)
#else
short WeaverStmtManager::OutputLinker(void* theRef, long theType, long index)
#endif
{
	short  x;
        int pipe_pos;
...
short WeaverStmtManager::GetOutputRef(short index,long type,void** ref)
{
	short id = 0;
	for (id=0;id<20;id++)
	{
...
void WeaverStmtManager::ClearData()
{
/*
...
void* WeaverStmtManager::Advance(long size)
{
	if (size >= stackSize ) return NULL;
	return ((char*)dataStack)+size;
}

short WeaverStmtManager::SetStatementSpaceSize( long size ) 
{
	stackSize = size;
	if ( dataStack == NULL ) {
		stackSize == 0;
...
short WeaverStmtManager::SetStatementBlobSize( long size ) 
{
	blob_size = size;
...
long WeaverStmtManager::GetStatementSpaceSize( void ) 
{
	return stackSize;
}


long WeaverStmtManager::GetStatementBlobSize( void ) 
{
	return blob_size;
}

short WeaverStmtManager::DelegateError(const char* state,const char* text,int code)
{
	errorlevel = 2;
...
short WeaverStmtManager::CheckLowerError( void ) 
{
        if ( !IsValid() ) return -1;
...
long WeaverStmtManager::align( long pointer ) 
{
...
short WeaverStmtManager::PassResults( JNIEnv* env )
{
	short   x;
	jclass		classID;
	jfieldID		fieldID; 
...
long WeaverStmtManager::StreamExec(char* statement)
{
        if ( !IsValid() ) return -1;
...
long WeaverStmtManager::PipeConnect(int pipeid,int streamid,pipefunc func)
{
...
long WeaverStmtManager::PipeDisconnect( int streamid ) 
{
       Pipe p = NULL;
        if ( !IsValid() ) return -1;
