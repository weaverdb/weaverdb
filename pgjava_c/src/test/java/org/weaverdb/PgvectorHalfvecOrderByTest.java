/*-------------------------------------------------------------------------
 *
 * ORDER BY halfvec L2 distance via JNI prepared Statement (WExec/WFetch path).
 *
 *-------------------------------------------------------------------------
 */
...
public class PgvectorHalfvecOrderByTest {
    ...
    public static void setup() throws Exception {
        try (DBReference conn = DBReferenceManager.connect("template1")) {
            exec(conn, "create table pv_jni_half_ob (id int, emb halfvec)");
            exec(conn, "insert into pv_jni_half_ob values (1, '[1,0,0]')");
            exec(conn, "insert into pv_jni_half_ob values (2, '[0,1,0]')");
            exec(conn, "insert into pv_jni_half_ob values (3, '[0,0,1]')");
            exec(conn, "insert into pv_jni_half_ob values (4, '[9,0,0]')");
        }
    }
    ...
    public void createHalfvecIvfflatIndex() throws Exception {
        try (DBReference conn = DBReferenceManager.connect("template1")) {
            exec(conn,
                    "create index pv_jni_half_ob_ivf on pv_jni_half_ob using ivfflat (emb halfvec_l2_ops) with (lists = 2)");
        }
    }
    ...
    public void createHalfvecHnswIndex() throws Exception {
        try (DBReference conn = DBReferenceManager.connect("template1")) {
            exec(conn,
                    "create index pv_jni_half_ob_hnsw on pv_jni_half_ob using hnsw (emb halfvec_l2_ops) with (m = 8, ef_construction = 32)");
        }
    }
