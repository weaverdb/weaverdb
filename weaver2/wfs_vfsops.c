...
#pragma ident	"@(#)zfs_vfsops.c	1.27	07/08/08 SMI"
...
int wfstype;
vfsops_t *wfs_vfsops = NULL;
static major_t wfs_major;
static minor_t wfs_minor;
static kmutex_t	wfs_dev_mtx;

static int wfs_mount(vfs_t *vfsp, vnode_t *mvp, struct mounta *uap, cred_t *cr);
static int wfs_umount(vfs_t *vfsp, int fflag, cred_t *cr);
static int wfs_mountroot(vfs_t *vfsp, enum whymountroot);
static int wfs_root(vfs_t *vfsp, vnode_t **vpp);
static int wfs_statvfs(vfs_t *vfsp, struct statvfs64 *statp);
static int wfs_vget(vfs_t *vfsp, vnode_t **vpp, fid_t *fidp);
static void wfs_freevfs(vfs_t *vfsp);

static const fs_operation_def_t wfs_vfsops_template[] = {
	VFSNAME_MOUNT,		{ .vfs_mount = wfs_mount },
	VFSNAME_UNMOUNT,	{ .vfs_unmount = wfs_umount },
	VFSNAME_ROOT,		{ .vfs_root = wfs_root },
	VFSNAME_STATVFS,	{ .vfs_statvfs = wfs_statvfs },
	VFSNAME_SYNC,		{ .vfs_sync = wfs_sync },
	VFSNAME_VGET,		{ .vfs_vget = wfs_vget },
	VFSNAME_FREEVFS,	{ .vfs_freevfs = wfs_freevfs },
	NULL,			NULL
};
 ...
 */
static uint32_t	wfs_active_fs_count = 0;
...
/*ARGSUSED*/
int
wfs_sync(vfs_t *vfsp, short flag, cred_t *cr)
{
	/*
...
static int
wfs_create_unique_device(dev_t *dev)
{
	major_t new_major;

	do {
		ASSERT3U(wfs_minor, <=, MAXMIN32);
		minor_t start = wfs_minor;
		do {
			mutex_enter(&wfs_dev_mtx);
			if (wfs_minor >= MAXMIN32) {
				/*
				 ...
				 */
				if (wfs_major == ddi_name_to_major(ZFS_DRIVER))
			...
			*dev = makedevice(wfs_major, wfs_minor);
			mutex_exit(&wfs_dev_mtx);
		} while (vfs_devismounted(*dev) && wfs_minor != start);
		if (wfs_minor == start) {
			/*
			...
			if ((new_major = getudev()) == (major_t)-1) {
				cmn_err(CE_WARN,
				    "wfs_mount: Can't get unique major "
				    "device number.");
				return (-1);
			}
			mutex_enter(&wfs_dev_mtx);
...
static int
wfs_domount(vfs_t *vfsp, char *osname, cred_t *cr)
{
	dev_t mount_dev;
	 ...
	 */
	wfsvfs = kmem_zalloc(sizeof (wfsvfs_t), KM_SLEEP);
	wfsvfs->w_vfs = vfsp;
	wfsvfs->w_parent = wfsvfs;
	wfsvfs->w_assign = TXG_NOWAIT;
	wfsvfs->w_max_blksz = SPA_MAXBLOCKSIZE;
	wfsvfs->w_show_ctldir = ZFS_SNAPDIR_VISIBLE;

	mutex_init(&wfsvfs->w_znodes_lock, NULL, MUTEX_DEFAULT, NULL);
	list_create(&wfsvfs->w_all_znodes, sizeof (znode_t),
	    offsetof(znode_t, w_link_node));
	rw_init(&wfsvfs->w_unmount_lock, NULL, RW_DEFAULT, NULL);
	rw_init(&wfsvfs->w_unmount_inactive_lock, NULL, RW_DEFAULT, NULL);
...
/*ARGSUSED*/
static int
wfs_mount(vfs_t *vfsp, vnode_t *mvp, struct mounta *uap, cred_t *cr)
{
	char		*osname;
	pathname_t	spn;
	int		error = 0;
	...
	error = wfs_domount(vfsp, osname, cr);
