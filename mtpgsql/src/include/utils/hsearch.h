 ...
 *
 * $Id: hsearch.h,v 1.1.1.1 2006/08/12 00:22:27 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#ifndef HSEARCH_H
...
typedef struct HTAB
{
	...
	void	   *(*alloc) (Size,void*);	/* memory allocator */
	void	   (*free) (void*,void*);	/* memory free */
...
typedef struct HASHCTL
{
	...
	long		max_dsize;		/* limit to dsize if directory size is
								 * limited */
	void	   *(*alloc) (Size,void*);	/* memory allocation function */
	void	   (*free) (void*,void*);	/* memory free function */
	HASHSEGMENT *dir;			/* directory of segment starts */
 ...
 */
PG_EXTERN HTAB *hash_create(const char *tabname, long nelem,
			HASHCTL *info, int flags);
PG_EXTERN void hash_destroy(HTAB *hashp);
PG_EXTERN void hash_stats(const char *where, HTAB *hashp);
PG_EXTERN void *hash_search(HTAB *hashp, void *keyPtr, HASHACTION action,
			bool *foundPtr);
PG_EXTERN void hash_seq_init(HASH_SEQ_STATUS *status, HTAB *hashp);
PG_EXTERN void *hash_seq_search(HASH_SEQ_STATUS *status);
PG_EXTERN long hash_estimate_size(long num_entries, long entrysize);
PG_EXTERN long hash_select_dirsize(long num_entries);

/*
 * prototypes for functions in hashfn.c
 */
PG_EXTERN long string_hash(void *key, int keysize);
PG_EXTERN long tag_hash(void *key, int keysize);
