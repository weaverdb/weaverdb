 ...
 *
 * $Id: parsenodes.h,v 1.1.1.1 2006/08/12 00:22:20 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#ifndef PARSENODES_H
...
{
	NodeTag		type;
	char	   *schemaname;			/* database to create */
} CreateSchemaStmt;
...
typedef struct VacuumStmt
{
	...
	bool		fixflags;		/* fix commit flags */
	bool		exclusive;		/* lock out all transactions before starting */
...
{
	NodeTag		type;
	char*			name;			/* the number of the parameter */
	TypeName   *typename;		/* the typecast */
	List	   *indirection;	/* array references */
} ParamName;
