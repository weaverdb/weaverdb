 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/rewrite/rewriteRemove.c,v 1.1.1.1 2006/08/12 00:21:23 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
RewriteGetRuleEventRel(char *rulename)
{
	...
	htup = SearchSysCacheTuple(RELOID,
							   ...
							   0, 0, 0);
	if (!HeapTupleIsValid(htup))
		elog(ERROR, "Class '%lu' not found", eventrel);
...
RemoveRewriteRule(char *ruleName)
{
	...
	eventRelationOidDatum = HeapGetAttr(tuple,
	 ...
	 */
	heap_delete(RewriteRelation, &tuple->t_self, NULL,NULL);
