 ...
 *
 *	$Id: be-dumpdata.c,v 1.1.1.1 2006/08/12 00:20:37 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
be_newportal(void)
{
	...
	if (be_portalcnt == 0)
		be_portaloid = newoid();
	be_portalcnt++;
	snprintf(buf, PortalNameLength, "be_%lu_%d", be_portaloid, be_portalcnt);
	...
	entry->portalcxt = (Pointer) MemoryContextGetCurrentContext();
