 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/libpq/be-pqexec.c,v 1.1.1.1 2006/08/12 00:20:37 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
	 ...
	 int nargs)
{
	...
	for (i = 0; i < nargs; i++)
	{
		if (args[i].len == VAR_LENGTH_ARG)
			...
			arg[i] = (char *)(long)(args[i].u.integer);
	}
