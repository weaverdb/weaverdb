 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/nodeMergejoin.c,v 1.1.1.1 2006/08/12 00:20:34 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
( \
	ExecStoreTuple(heap_copytuple((innerTupleSlot)->val), \
				   (mergestate)->mj_MarkedTupleSlot,false) \
)
	...
	for (;;)
	{
		...
		switch (mergestate->mj_JoinState)
		{
			...
			case EXEC_MJ_INITIALIZE:
				...
				ExecSetSlotDescriptor(mergestate->mj_MarkedTupleSlot,innerTupleSlot->ttc_tupleDescriptor);
...
ExecInitMergeJoin(MergeJoin *node, EState *estate)
{
	 ...
	 * ----------------
	 */
	ExecAssignNodeBaseInfo(estate, &mergestate->jstate);
...
#define MERGEJOIN_NSLOTS 2
	 ...
	 * ----------------
	 */
	ExecInitNode(outerPlan((Plan *) node), estate);
	ExecInitNode(innerPlan((Plan *) node), estate);
...
ExecEndMergeJoin(MergeJoin *node)
{
	 ...
	 * ----------------
	 */
	ExecEndNode((Plan *) innerPlan((Plan *) node));
	ExecEndNode((Plan *) outerPlan((Plan *) node));
...
void
ExecReScanMergeJoin(MergeJoin *node, ExprContext *exprCtxt)
{
