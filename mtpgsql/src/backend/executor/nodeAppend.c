 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/nodeAppend.c,v 1.1.1.1 2006/08/12 00:20:31 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/* INTERFACE ROUTINES
...
ExecInitAppend(Append *node, EState *estate)
{
	 ...
	 * ----------------
	 */
	ExecAssignNodeBaseInfo(estate, &appendstate->cstate);
...
#define APPEND_NSLOTS 1
	...
	for (i = 0; i < nplans; i++)
	{
		...
		initialized[i] = ExecInitNode(initNode, estate);
...
ExecProcAppend(Append *node)
{
	...
	if (!TupIsNull(result))
	{
		 ...
		 * ----------------
		 */
		return ExecStoreTuple(result->val, result_slot,false);
	}
	else
	{
		/* ----------------
...
ExecEndAppend(Append *node)
{
	...
	for (i = 0; i < nplans; i++)
	{
		if (initialized[i] == TRUE)
			ExecEndNode((Plan *) nth(i, appendplans));
	}
...
void
ExecReScanAppend(Append *node, ExprContext *exprCtxt)
{
	...
	for (i = 0; i < nplans; i++)
	{
		...
		{
			exec_append_initialize_next(node);
			ExecReScan((Plan *) rescanNode, exprCtxt);
		}
	}
	appendstate->as_whichplan = 0;
	exec_append_initialize_next(node);
