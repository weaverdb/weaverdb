 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/functions.c,v 1.1.1.1 2006/08/12 00:20:30 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <string.h>
				 ...
				 bool *isnullP)
{
	...
	val = HeapGetAttr(tup, attrno, TD, isnullP);
	if (*isnullP)
		return (Datum) NULL;
					 ...
					 char *args[])
{
	...
	queryTree_list = pg_parse_and_rewrite(fcache->src, fcache->argOidVect,NULL,
										  nargs, FALSE);
					 ...
					 TupleTableSlot *resultSlot)
{
	...
	return ExecStoreTuple(newTuple, funcSlot, false);
}
