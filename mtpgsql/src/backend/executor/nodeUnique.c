 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/nodeUnique.c,v 1.1.1.1 2006/08/12 00:20:35 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
/*  switch to the memory context of the state for the copy  */
        {
            MemoryContext old = MemoryContextSwitchTo(GetMemoryContext(uniquestate));
            uniquestate->priorTuple = heap_copytuple(slot->val);
            MemoryContextSwitchTo(old);
        }

	ExecStoreTuple(slot->val,resultTupleSlot,false);
...
ExecInitUnique(Unique *node, EState *estate)
{
	...
	uniquestate->priorTuple = NULL;
        uniquestate->save_cxt = MemoryContextGetCurrentContext();
	/* ----------------
	 ...
	 * ----------------
	 */
	ExecAssignNodeBaseInfo(estate, &uniquestate->cstate);
...
void
ExecReScanUnique(Unique *node, ExprContext *exprCtxt)
{
	UniqueState *uniquestate = node->uniquestate;
