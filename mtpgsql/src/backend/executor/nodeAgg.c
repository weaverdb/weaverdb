	...
	if (aggref->aggdistinct)
	{
								  ...
								  &newVal, &isNull))
		{
			...
			if (haveOldVal)
			{
				...
				equal = PointerGetDatum(FMGR_PTR2(&peraggstate->equalfn, oldVal,
																 newVal));
...
void
ExecReScanAgg(Agg *node, ExprContext *exprCtxt)
{
	AggState   *aggstate = node->aggstate;
