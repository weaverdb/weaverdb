 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/nodeHashjoin.c,v 1.1.1.1 2006/08/12 00:20:32 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <sys/types.h>
...
static TupleTableSlot *ExecHashJoinOuterGetTuple(Plan *node,
...
//	dir = estate->es_direction;
	...
	for (;;)
	{
		...
		{
			outerTupleSlot = ExecHashJoinOuterGetTuple(outerNode,hjstate);
			if (TupIsNull(outerTupleSlot))
			{
			...
			hjstate->hj_CurBucketNo = ExecHashGetBucket(hashtable, econtext,outerVar);
		...
		for (;;)
		{
			 ...
			 */
			inntuple = ExecStoreTuple(curtuple,hjstate->hj_HashTupleSlot,false);	/* don't pfree this tuple */
...
ExecInitHashJoin(HashJoin *node, EState *estate)
{
	 ...
	 * ----------------
	 */
	ExecAssignNodeBaseInfo(estate, &hjstate->jstate);
...
#define HASHJOIN_NSLOTS 2
	...
	ExecInitNode(outerNode, estate);
	ExecInitNode((Plan *) hashNode, estate);
	...
	ExecSetSlotDescriptor(hjstate->hj_OuterTupleSlot,ExecGetTupType(outerNode));
...
ExecEndHashJoin(HashJoin *node)
{
	...
	ExecEndNode(innerPlan((Plan *) node));
...
ExecHashJoinOuterGetTuple(Plan *node, HashJoinState *hjstate)
{
						  ...
						  TupleTableSlot *tupleSlot)
{
	...
	heapTuple->t_datamcxt = MemoryContextGetCurrentContext();
	heapTuple->t_datasrc = NULL;
	heapTuple->t_info = 0;
	...
	if (nread != (size_t) htup.t_len)
		elog(ERROR, "Read from hashjoin temp file failed");
	return ExecStoreTuple(heapTuple, tupleSlot, true);
}
...
ExecHashJoinNewBatch(HashJoinState *hjstate)
{
	 ...
	 * start reading them.
	 */
	if (BufFileSeek(hashtable->outerBatchFile[newbatch - 1], 0L, SEEK_SET))
		elog(ERROR, "Failed to rewind hash temp file");

	innerFile = hashtable->innerBatchFile[newbatch - 1];

	if (BufFileSeek(innerFile,  0L, SEEK_SET))
		elog(ERROR, "Failed to rewind hash temp file");
...
void
ExecHashJoinSaveTuple(HeapTuple heapTuple,BufFile *file)
{
	size_t		written;
...
void
ExecReScanHashJoin(HashJoin *node, ExprContext *exprCtxt)
{
	...
	if (((Plan *) node)->righttree->chgParam == NULL)
		ExecReScan(((Plan *) node)->righttree, exprCtxt);
}
