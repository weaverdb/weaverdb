 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/nodeSubplan.c,v 1.1.1.1 2006/08/12 00:20:35 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
ExecSubPlan(SubPlan *node, List *pvar, ExprContext *econtext, bool *isNull)
{
	...
	ExecReScan(plan, (ExprContext *) NULL);
	...
	*isNull = false;
		 ...
		 slot = ExecProcNode(plan))
	{
		...
		if (subLinkType == EXPR_SUBLINK)
		{
			...
			node->curTuple = tup;
			result = HeapGetAttr(tup, col, tdesc, isNull);
		...
		foreach(lst, sublink->oper)
		{
			...
			con->constvalue = HeapGetAttr(tup, col, tdesc,
										   &(con->constisnull));
...
ExecInitSubPlan(SubPlan *node, EState *estate)
{
	...
	if (!ExecInitNode(node->plan, sp_estate))
		return false;
...
ExecSetParamPlan(SubPlan *node)
{
		 ...
		 slot = ExecProcNode(plan))
	{
		...
		foreach(lst, node->setParam)
		{
			...
			prm->execPlan = NULL;
			prm->value = HeapGetAttr(tup, i, tdesc, &(prm->isnull));
			i++;
		}
	}
...
void
ExecReScanSetParamPlan(SubPlan *node, Plan* upper)
{
	Plan	   *plan = node->plan;
	List	   *lst;
	...
	upper->chgParam = nconc(upper->chgParam, listCopy(node->setParam));
