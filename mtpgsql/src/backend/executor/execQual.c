 ...
 * 
 * These are functions which return the value of the named attribute out of the
 * tuple from the arg slot.  User defined C functions which take a tuple as
 * an argument are expected to use this.  Ex: overpaid(EMP) might call
 * GetAttributeByNum().
 */
		  ...
		  bool * isNull, bool * freeable)
{
	...
	if (isNull == NULL)
		elog(ERROR, "null pointer passed in GetAttributeByNum");

	if (TupIsNull(slot)) {
		*isNull = true;
		return (char *) NULL;
	}
	retval = HeapGetAttr(slot->val,
			     attrno,
			     slot->ttc_tupleDescriptor,
			     isNull);

	if (slot->ttc_tupleDescriptor->attrs[attrno - 1]->attstorage == 'e' && !(*isNull) && ISINDIRECT(retval)) {
		retval = PointerGetDatum(rebuild_indirect_blob(retval));
		if (freeable)
			*freeable = true;
	}
	if (*isNull)
...
char           *
GetAttributeByName(TupleTableSlot * slot, char *attname, bool * isNull, bool * freeable)
{
	AttrNumber      attrno;
	TupleDesc       tupdesc;
	Datum           retval;
	...
	if (tupdesc->attrs[attrno - 1]->attstorage == 'e' && !(*isNull) && ISINDIRECT(retval)) {
		retval = PointerGetDatum(rebuild_indirect_blob(retval));
		if (freeable)
			*freeable = true;
	}
