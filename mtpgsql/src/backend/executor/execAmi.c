 ...
 *
 *	$Id: execAmi.c,v 1.1.1.1 2006/08/12 00:20:26 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
			  ...
			  Pointer *returnScanDesc)	/* return */
{
	...
	if (relation == NULL)
		elog(ERROR, "ExecOpenScanR: failed to open relation %lu", relOid);
...
ExecCloseR(Plan *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_DelegatedSeqScan:
                        state = ((DelegatedSeqScan *) node)->scan.scanstate;
			break;
		...
		case T_DelegatedIndexScan:
			state = ((DelegatedIndexScan *) node)->scan.scanstate;
			break;
...
ExecReScan(Plan *node, ExprContext *exprCtxt)
{
	...
	{
		case T_SeqScan:
			ExecSeqReScan((SeqScan *) node, exprCtxt);
			break;

		case T_DelegatedSeqScan:
			ExecDelegatedSeqReScan((DelegatedSeqScan *) node, exprCtxt);
			break;

		case T_IndexScan:
			ExecIndexReScan((IndexScan *) node, exprCtxt);
			break;

		case T_DelegatedIndexScan:
			ExecDelegatedIndexReScan((DelegatedIndexScan *) node, exprCtxt);
			break;

		case T_Material:
			ExecMaterialReScan((Material *) node, exprCtxt);
			break;

		case T_NestLoop:
			ExecReScanNestLoop((NestLoop *) node, exprCtxt);
			break;

		case T_HashJoin:
			ExecReScanHashJoin((HashJoin *) node, exprCtxt);
			break;

		case T_Hash:
			ExecReScanHash((Hash *) node, exprCtxt);
			break;

		case T_Agg:
			ExecReScanAgg((Agg *) node, exprCtxt);
			break;

		case T_Group:
			ExecReScanGroup((Group *) node, exprCtxt);
			break;

		case T_Result:
			ExecReScanResult((Result *) node, exprCtxt);
			break;

		case T_Unique:
			ExecReScanUnique((Unique *) node, exprCtxt);
			break;

		case T_Sort:
			ExecReScanSort((Sort *) node, exprCtxt);
			break;

		case T_MergeJoin:
			ExecReScanMergeJoin((MergeJoin *) node, exprCtxt);
			break;

		case T_Append:
			ExecReScanAppend((Append *) node, exprCtxt);
			break;

		case T_TidScan:
			ExecTidReScan((TidScan *) node, exprCtxt);
			break;
