 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/executor/nodeSort.c,v 1.1.1.1 2006/08/12 00:20:33 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
ExecInitSort(Sort *node, EState *estate)
{
	 ...
	 * ----------------
	 */
	ExecAssignNodeBaseInfo(estate, &sortstate->csstate.cstate);
...
#define SORT_NSLOTS 1
...

	{
		...
		sortstate->tuplesortstate = tuplesort_begin_heap(tupDesc, keycount, sortkeys,
											  true /* randomAccess */ );
	}
	return TRUE;
}
...
void
ExecReScanSort(Sort *node, ExprContext *exprCtxt)
{
	SortState  *sortstate = node->sortstate;
