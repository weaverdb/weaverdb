 ...
 *		ExecEndTee
 *
 *	$Id: nodeTee.c,v 1.1.1.1 2006/08/12 00:20:36 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
#define TEE_NSLOTS 2
	...
	*/
	...
	teeState->tee_mcxt = AllocSetContextCreate(MemoryContextGetCurrentContext(),
												   "TeeMemoryContext",
...
/*  I'm not sure if this is valid anymore.  I significantly changed 
 *  ExecStoreTuple to store in memory rather than referencing a 
 *  buffer page...comment out the call for now.
...
*/
		...
		ExecSetSlotDescriptor(slot,RelationGetDescr(bufferRel));
		result = ExecStoreTuple(heapTuple,slot,false); /* don't free stuff from
										 * heap_getnext */
