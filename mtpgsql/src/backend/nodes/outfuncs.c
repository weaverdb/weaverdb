 ...
 *
 *	$Header: /cvs/weaver/mtpgsql/src/backend/nodes/outfuncs.c,v 1.1.1.1 2006/08/12 00:20:42 synmscott Exp $
 *
 * NOTES
...
/*
 *	DelegatedSeqScan is a subclass of Scan
 */
static void
_outDelegatedSeqScan(StringInfo str, DelegatedSeqScan *node)
{
	appendStringInfo(str, " SEQSCAN ");
	_outPlanInfo(str, (Plan *) node);
...
static void
_outDelegatedIndexScan(StringInfo str, DelegatedIndexScan *node)
{
	appendStringInfo(str, " DELEGATEDINDEXSCAN ");
	_outPlanInfo(str, (Plan *) node);

	appendStringInfo(str, " :scanrelid %u :indxid %u ", node->scan.scanrelid, node->indexid);
...
_outEState(StringInfo str, EState *node)
{
	...
	appendStringInfo(str, " :result_relation_info @ 0x%x ",
					 (long) (node->es_result_relation_info));
}
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_DelegatedSeqScan:
				_outDelegatedSeqScan(str, obj);
				break;
			case T_IndexScan:
				_outIndexScan(str, obj);
				break;
			case T_DelegatedIndexScan:
				_outDelegatedIndexScan(str, obj);
				break;
			case T_TidScan:
				_outTidScan(str, obj);
				break;
