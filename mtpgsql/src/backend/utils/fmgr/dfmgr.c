 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/fmgr/dfmgr.c,v 1.1.1.1 2006/08/12 00:21:58 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <sys/types.h>
...
fmgr_dynamic(Oid procedureId, int *pronargs)
{
	...
	probinattr = HeapGetAttr(procedureTuple,
							  Anum_pg_proc_probin,
	...
	prosrcattr = HeapGetAttr(procedureTuple,
							  Anum_pg_proc_prosrc,
							  RelationGetDescr(rel), &isnull);
