 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/hash/dynahashnew.c,v 1.1.1.1 2006/08/12 00:22:00 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
{
	if (!GetEnv()->DynaHashCxt) {
		GetEnv()->DynaHashCxt = CreateGlobalMemory("DynaHash");
	}
	return (long *)
		MemoryContextAlloc((MemoryContext) GetEnv()->DynaHashCxt, size);
}

static void
DynaHashFree(Pointer ptr)
{
	MemoryContextFree((MemoryContext) GetEnv()->DynaHashCxt, ptr);
}
...
hash_create(int nelem, HASHCTL *info, int flags)
{
	...
	if ( flags & HASH_MT ) {
		hashp->gate = (mutex_t*)MEM_ALLOC((unsigned long)sizeof(mutex_t));
		mutex_init(hashp->gate,USYNC_THREAD,0);
	}
...
hash_destroy(HTAB *hashp)
{
	if (hashp != NULL)
	{
		...
		if ( hashp->gate != NULL ) MEM_FREE((char*)hashp->gate);
	...
	switch (action)
	{
		case HASH_ENTER:
			if (currIndex != INVALID_INDEX) {
				if ( hashp->gate != NULL ) mutex_unlock(hashp->gate);
				return &(curr->key);
			}
			break;
		case HASH_REMOVE:
...
hash_seq(HTAB *hashp)
{
	...
	if ( hashp->gate != NULL ) mutex_lock(hashp->gate);
	hctl = hashp->hctl;
