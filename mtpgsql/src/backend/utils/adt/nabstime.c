 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/adt/nabstime.c,v 1.1.1.1 2006/08/12 00:21:46 synmscott Exp $
 *
 * NOTES
 *
 *-------------------------------------------------------------------------
...
abstime2tm(AbsoluteTime time, int *tzp, struct tm * tm, char *tzn)
{
	...
	if (tzp != NULL)
#ifdef MACOSX
		memcpy(&tx,localtime(&reserve),sizeof(struct tm));
#else
		localtime_r(&reserve,&tx);
#endif
	else
	{
#ifdef MACOSX
		memcpy(&tx,gmtime(&reserve),sizeof(struct tm));
#else
		gmtime_r(&reserve,&tx);
#endif
	};
