 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/adt/not_in.c,v 1.1.1.1 2006/08/12 00:21:46 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
int4notin(int32 not_in_arg, char *relation_and_attr)
{
	...
	relation = (char *) strtok_r(my_copy, ".",&attribute);
	if (attribute == NULL)
	...
	while (HeapTupleIsValid(current_tuple = heap_getnext(scan_descriptor)))
	{
		value = HeapGetAttr(current_tuple,
							 (AttrNumber) attrid,
