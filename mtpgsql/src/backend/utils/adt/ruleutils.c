...
pg_get_ruledef(NameData *rname)
{
	...
	if (SPI_GetInfo()->SPI_processed != 1)
	{
		...
		SETVARSIZE(ruledef,VARHDRSZ + 1);
		VARDATA(ruledef)[0] = '-';
		return ruledef;
	}

	ruletup = SPI_GetInfo()->SPI_tuptable->vals[0];
	rulettc = SPI_GetInfo()->SPI_tuptable->tupdesc;
