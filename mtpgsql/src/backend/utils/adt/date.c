 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/adt/date.c,v 1.1.1.1 2006/08/12 00:21:39 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <limits.h>
...
#ifdef MACOSX
		memcpy(&tx,localtime(&utime),sizeof(tx));
#else
		localtime_r(&utime,&tx);
#endif
...
datetimetz_timestamp(DateADT date, TimeTzADT *time)
{
	...
	if (!PointerIsValid((long)date) || !PointerIsValid(time))
		TIMESTAMP_INVALID(*result);
	else
	{
