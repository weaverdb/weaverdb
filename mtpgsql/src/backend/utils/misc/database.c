 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/misc/database.c,v 1.1.1.1 2006/08/12 00:22:02 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
Oid 
GetDatabaseInfo(const char* name,char* path) {
	Relation dbrel;
	ScanKeyData   key;
	Name			dbname;
    HeapScanDesc  scan;
	...
	desc = RelationGetDescr(dbrel);

    ScanKeyEntryInitialize(&key,0,Anum_pg_database_datname,F_NAMEEQ,NameGetDatum(dbname));
    scan = (HeapScanDesc)heap_beginscan(dbrel,SnapshotSelf,1,&key);
    tup = (HeapTuple)heap_getnext(scan);
        
    if ( HeapTupleIsValid(tup) ) {
    	bool isnull = false;
    	dbid = DatumGetObjectId(HeapGetAttr(tup,ObjectIdAttributeNumber,desc,&isnull));
    	strncpy(path,textout((struct varlena *)HeapGetAttr(tup,Anum_pg_database_datpath,desc,&isnull)),255);
	}
...
GetRawDatabaseInfo(const char *name, Oid *db_id, char *path)
{
	...
	memset(dbfname,0,strlen(DataDir) + strlen(DatabaseRelationName) + 2);
	strncpy(dbfname,DataDir,strlen(DataDir));
	memset(dbfname+strlen(DataDir),SEP_CHAR,1);
	strncpy(dbfname+strlen(DataDir)+1,DatabaseRelationName,strlen(DatabaseRelationName));
 
/*	elog(DEBUG,"database home %s", dbfname);    */

#ifndef __CYGWIN32__
	if ((dbfd = PathNameOpenFile(dbfname, O_RDONLY, 0)) < 0) {
#else
	if ((dbfd = PathNameOpenFile(dbfname, O_RDONLY | O_BINARY, 0)) < 0) {
#endif
		elog(DEBUG, "cannot open %s: %s", dbfname, strerror(errno));
		GetEnv()->DatabaseId = InvalidOid;
		return;
	}
	pfree(dbfname);
	...
	ph = (PageHeader) pg;
        ...
        FilePin(dbfd,0);
	while ((nbytes = FileRead(dbfd, pg, BLCKSZ)) == BLCKSZ)
	{
                FileUnpin(dbfd,0);
