...
#define AllocFreeInfo(_cxt, _chunk, file, line, func) \
			DebugMemory("AllocFree", \
				(_cxt)->header.name, (_chunk), (_chunk)->size, file, line, func)
#define AllocAllocInfo(_cxt, _chunk, file, line, func) \
			DebugMemory("AllocAlloc", \
				(_cxt)->header.name, (_chunk), (_chunk)->size, file, line, func)
#endif
					  ...
					  Size maxBlockSize)
{
	...
	if (minContextSize > ALLOC_BLOCKHDRSZ + ALLOC_CHUNKHDRSZ)
	{
		...
		block = (AllocBlock) os_malloc(blksize);
		if (block == NULL)
		{
...
#endif
{
	...
	if (chunk->size > ALLOC_CHUNK_LIMIT)
	{
		...
		if (block == NULL)
			elog(FATAL, "AllocSetFree: cannot find block containing chunk %p",
				 chunk);
...
AllocSetRealloc(MemoryContext context, void *pointer, Size size)
{
	...
	if (oldsize > ALLOC_CHUNK_LIMIT)
	{
		...
		block = (AllocBlock) os_realloc(block, blksize);
		if (block == NULL)
		{
			MemoryContextStats(MemoryContextGetTopContext());
	...
	else
	{
		...
		if (size <= ALLOC_CHUNK_LIMIT)
		{
			...
			chunk_end = ((char *)pointer) + (oldsize);
			if (chunk_end == block->freeptr)
			{
				...
				{
/*
                                    printf("realloc reuse endblock %d\n",delta);
*/
...
/*
                ...
                if ( (long)newPointer < (long)pointer && (long)((char*)newPointer + size) > (long)pointer ) {
                    abort();
                }
*/
