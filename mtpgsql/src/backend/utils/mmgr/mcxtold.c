 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/mmgr/mcxtold.c,v 1.1.1.1 2006/08/12 00:22:03 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
int	MemoryContextEnableCount = 0;
...
EnableMemoryContext(bool on)
{
	...
	fprintf(stdout,"start memory enable\n");
	...
	fprintf(stdout,"OrderedSetInit\n");
		OrderedElemPushInto(&TopGlobalMemoryData.elemData,
							ActiveGlobalMemorySet);
	fprintf(stdout,"OrderedElemPushInto\n");

		/* initialize CurrentMemoryContext */
			fprintf(stdout,"done with memory\n");

		GetEnv()->CurrentMemoryContext = TopMemoryContext; 
		MemoryContextEnableCount = TRUE;
	...
	{							/* cleanup */
		...
		MemoryContextEnableCount = FALSE;
	}
...
{		
	...
	if ( GetEnv() == NULL ) return TopMemoryContext;
	
	old = GetEnv()->CurrentMemoryContext;
	GetEnv()->CurrentMemoryContext = context;
	return old;
}
...
CreateGlobalMemory(char *name)	/* XXX MemoryContextName */
{
	...
	fprintf(stdout,"creating global memory");
