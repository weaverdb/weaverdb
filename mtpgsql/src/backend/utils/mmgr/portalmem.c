 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/utils/mmgr/portalmem.c,v 1.1.1.1 2006/08/12 00:22:03 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
typedef struct PortalInfo {
	HTAB* PortalHashTable;
	MemoryContext PortalMemory;
} PortalInfo;

static SectionId portal_id = SECTIONID("PORT");
/*
static HTAB *PortalHashTable = NULL;
*/
#define PortalHashTableLookup(NAME, PORTAL) \
do { \
	PortalInfo*     pdinfo = GetEnvSpace(portal_id); \
	PortalHashEnt *hentry; char key[MAX_PORTALNAME_LEN]; \
	\
	MemSet(key, 0, MAX_PORTALNAME_LEN); \
	snprintf(key, MAX_PORTALNAME_LEN - 1, "%s", NAME); \
	hentry = (PortalHashEnt*)hash_search(pdinfo->PortalHashTable, \
										 key, HASH_FIND, NULL); \
	if (hentry) \
...
{
	HASHCTL		ctl;
	PortalInfo* pinfo = AllocateEnvSpace(portal_id,sizeof(PortalInfo));

	pinfo->PortalMemory = AllocSetContextCreate(MemoryContextGetTopContext(),
										 "PortalMemory",
	 ...
	 */
	pinfo->PortalHashTable = hash_create("Portal hash", PORTALS_PER_USER,
								  &ctl, HASH_ELEM);
}
...
{
	Portal		portal;
	PortalInfo*     pinfo = GetEnvSpace(portal_id);

        if ( pinfo == NULL ) {
            elog(ERROR,"portals are not turned on");
        }
...
CreatePortal(char *name)
{
	...
	portal = (Portal) MemoryContextAlloc(pinfo->PortalMemory, sizeof *portal);

	/* initialize portal name */
	portal->name = MemoryContextStrdup(pinfo->PortalMemory, name);

	/* initialize portal heap context */
	portal->heap = AllocSetContextCreate(pinfo->PortalMemory,
										 "PortalHeapMemory",
										 ALLOCSET_DEFAULT_MINSIZE,
										 ALLOCSET_DEFAULT_INITSIZE,
										 ALLOCSET_DEFAULT_MAXSIZE);
...
AtEOXact_portals(void)
{
	...
	hash_seq_init(&status, pinfo->PortalHashTable);
