 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/catalog/pg_type.c,v 1.1.1.1 2006/08/12 00:20:13 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
typedef struct TypeInfo {
	ScanKeyData		typeKey1[1];
	ScanKeyData		typeKey2[1];
} TypeInfo;

static SectionId type_id = SECTIONID("TYPE");

#ifdef TLS
TLS TypeInfo* type_globals = NULL;
#else
#define type_globals GetEnv()->type_globals
#endif
 ...
 */
 	static ScanKeyData typeKeyTemp[1] = {
		{0, Anum_pg_type_typname, F_NAMEEQ}
	};

 #define  typeKey1   GetTypeInfo()->typeKey1
						...
						bool *defined)
{
	...
	memcpy(typeKey1,typeKeyTemp,sizeof(typeKeyTemp));

	/* ----------------
	 *	initialize the scan key and begin a scan of pg_type
	 * ----------------
	 */
	fmgr_info(F_NAMEEQ, &typeKey1[0].sk_func);
	typeKey1[0].sk_nargs = typeKey1[0].sk_func.fn_nargs;
	typeKey1[0].sk_argument = PointerGetDatum(typeName);
...
#define typeKey2  GetTypeInfo()->typeKey2
...
*/
	memcpy(typeKey2,typeKeyTemp,sizeof(typeKeyTemp));
	
	fmgr_info(F_NAMEEQ, &typeKey2[0].sk_func);
	typeKey2[0].sk_nargs = typeKey2[0].sk_func.fn_nargs;
	...
	typeKey2[0].sk_argument = PointerGetDatum(typeName);
...
GetTypeInfo(void)
{
    TypeInfo* info = type_globals;
    if ( info == NULL ) {
        info = AllocateEnvSpace(type_id,sizeof(TypeInfo));
        type_globals = info;
    }
    return info;
}
