...
relpath(const char *relname)
{
	 ...
	 */
  /*  Get the database Path and add it     */ 
        bufsize = strlen(env->DatabasePath) + 2 + sizeof(NameData);
        path = (char *)palloc(bufsize);
	snprintf(path, bufsize, "%s%c%s",env->DatabasePath, SEP_CHAR, relname);
	return path;
}
			  ...
			  Oid dbid, Oid relid)
{
	...
	else if (dbid == GetEnv()->DatabaseId)
	{
		/* XXX why is this inconsistent with relpath() ? */
		path = (char *) palloc(strlen(GetDatabasePath()) + sizeof(NameData) + 2);
		sprintf(path, "%s%c%s",GetDatabasePath(), SEP_CHAR, relname);
	}
	else
	{
		...
		Oid 		id;
			...
			id = dbid;
			strncpy(dbpathtmp,dbname,MAXPGPATH);
		} else {
