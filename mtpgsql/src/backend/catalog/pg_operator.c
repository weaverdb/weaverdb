...
typedef struct OperatorInfo {
	ScanKeyData		opKey1[3];
	ScanKeyData		opKey2[3];
	ScanKeyData		opKey3[1];
} OperatorInfo;

static SectionId operator_id = SECTIONID("OPER");

#ifdef TLS
TLS OperatorInfo* operator_globals = NULL;
#else
#define operator_globals GetEnv()->operator_globals
#endif

static OperatorInfo* GetOperatorInfo(void);
							...
							bool *defined)
{
	...
	memcpy(opKey1,opKeyTemp1,sizeof(opKeyTemp1));	

	fmgr_info(F_NAMEEQ, &opKey1[0].sk_func);
	fmgr_info(F_OIDEQ, &opKey1[1].sk_func);
	fmgr_info(F_OIDEQ, &opKey1[2].sk_func);
	opKey1[0].sk_nargs = opKey1[0].sk_func.fn_nargs;
	opKey1[1].sk_nargs = opKey1[1].sk_func.fn_nargs;
	opKey1[2].sk_nargs = opKey1[2].sk_func.fn_nargs;

	/* ----------------
	 *	form scan key
	 * ----------------
	 */
	opKey1[0].sk_argument = PointerGetDatum(operatorName);
	opKey1[1].sk_argument = ObjectIdGetDatum(leftObjectId);
	opKey1[2].sk_argument = ObjectIdGetDatum(rightObjectId);
	 ...
	 * ----------------
	 */
	tup = heap_getnext(pg_operator_scan);
 ...
 */
	...
	static ScanKeyData opKeyTemp2[3] = {
		{0, Anum_pg_operator_oprname, F_NAMEEQ},
		{0, Anum_pg_operator_oprleft, F_OIDEQ},
		{0, Anum_pg_operator_oprright, F_OIDEQ},
	};

#define  opKey2		GetOperatorInfo()->opKey2	
			...
			char *rightSortName)
{
	...
	memcpy(opKey2,opKeyTemp2,sizeof(opKeyTemp2));

	fmgr_info(F_NAMEEQ, &opKey2[0].sk_func);
	fmgr_info(F_OIDEQ, &opKey2[1].sk_func);
	fmgr_info(F_OIDEQ, &opKey2[2].sk_func);
	opKey2[0].sk_nargs = opKey2[0].sk_func.fn_nargs;
	opKey2[1].sk_nargs = opKey2[1].sk_func.fn_nargs;
	opKey2[2].sk_nargs = opKey2[2].sk_func.fn_nargs;
	...
	if (operatorObjectId)
	{
		opKey2[0].sk_argument = PointerGetDatum(operatorName);
		opKey2[1].sk_argument = ObjectIdGetDatum(leftTypeId);
		opKey2[2].sk_argument = ObjectIdGetDatum(rightTypeId);
 ...
 */
	...
	static ScanKeyData opKeyTemp3[1] = {
		{0, ObjectIdAttributeNumber, F_OIDEQ},
	};

#define  opKey3 GetOperatorInfo()->opKey3
...
OperatorUpd(Oid baseId, Oid commId, Oid negId)
{
	...
	memcpy(opKey3,opKeyTemp3,sizeof(opKeyTemp3));
	
	fmgr_info(F_OIDEQ, &opKey3[0].sk_func);
	opKey3[0].sk_nargs = opKey3[0].sk_func.fn_nargs;
	...
	opKey3[0].sk_argument = ObjectIdGetDatum(commId);
	...
	opKey3[0].sk_argument = ObjectIdGetDatum(negId);
...
{
    OperatorInfo* info = operator_globals;
    if ( info == NULL ) {
        info = AllocateEnvSpace(operator_id,sizeof(OperatorInfo));
        operator_globals = info;
    }
    return info;
}
