 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/catalog/pg_aggregate.c,v 1.1.1.1 2006/08/12 00:20:12 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
AggNameGetInitVal(char *aggName, Oid basetype, int xfuncno, bool *isNull)
{
	...
	{
		pfree(strInitVal);
		elog(ERROR, "AggNameGetInitVal: cache lookup failed on aggregate transition function return type %lu", transtype);
	}
