...
crc64
checksum_block(unsigned char* block,int len)
{
/*  add the last 8 bytes of every 1K to the checksum 
    we are mainly trying to detect torn pages so 
    this should be ok.  Checksumming the whole block has proved
    expensive
*/
    int pos = 1024;
    pg_crc64 checksum;
    INIT_CRC64(checksum);
    while (pos <= len) {
        COMP_CRC64(checksum,block + (pos - 64),64);
        pos += 1024;
    }
    FINI_CRC64(checksum);
    return checksum.crc0;
}
...
bool
eq_crc64(crc64 c1,crc64 c2)
{
    pg_crc64   s1,s2;
    s1.crc0 = c1;
    s2.crc0 = c2;
    return EQ_CRC64(s1,s2);
}
