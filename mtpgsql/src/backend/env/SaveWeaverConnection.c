...
WParsingFunc(OpaqueWConn conn, const char *smt)
{
        ...
        if ( !pthread_equal(connection->transaction_owner,pthread_self()) ) {
            elog(ERROR,"transaction is owned by thread %d, cannot make call from this context",connection->transaction_owner);
        }
        if ( stmtlen > 8191 ) {
            elog(ERROR, "parsed statement too long");
        }
...
/* already hit the end of the query, throw an error  */
        coded_elog(ERROR,1405,"fetch past end of cursor");
    }
    ...
    if (TupIsNull(slot)) {
        coded_elog(ERROR,1403,"no more data");
    } else {
        ...
        while (connection->output[pos].index != 0 && pos <= MAX_ARGS) {
            ...
            /* field was not transfered, try and coerce to see if it should someday  */
                ...
                *connection->output[pos].isNull = 0;  /*  the value is not null */
            } else {
                *connection->output[pos].isNull = 1;  /*  the value is null */
            }
            pos++;
        }
        ExecClearTuple(slot);
...
extern Pipe WPipeConnect(OpaqueWConn conn, int pipeid, int sid, int size, pipefunc func)
{
    ...
    if ( sid == 0 ) {
        ...
        std->cursor.ptr = 0;
        std->cursor.end = 0;
        std->cursor.size = PQ_BUFFER_SIZE;
        pipe = &std->pipe;
    } else if ( sid == 1 ) {
        StdComm* std = MemoryContextAlloc(MemoryContextGetTopContext(),sizeof(StdComm));
        GetEnv()->pipeout = std;
...
WPipeDisconnect(OpaqueWConn conn, Pipe pipe)
{
    ...
    if ( pipe == env->pipein || env->pipeout == env->pipeout ) {
        ...
        if (comm->cursor.ptr < comm->cursor.end) {
            if ( comm->pipe.pipe(comm->pipe.pipeid, comm->pipe.streamid, comm->cursor.buffer, comm->cursor.ptr, comm->cursor.end - comm->cursor.ptr) ==PIPING_ERROR ) {
    ...
    for (k = 0; k < connection->nargs; k++) {
        ...
        switch (connection->input[k].ctype) {
            ...
            case STREAMINGOID:
                paramLI->value = (Datum) palloc(connection->input[k].varSize);
                memcpy((char *) paramLI->value, connection->input[k].target, connection->input[k].varSize);
                paramLI->length = (Size)(-1);/* -1 == variable size slot */ 
                break;
            case VARCHAROID:
            case TEXTOID:
            case BLOBOID:
