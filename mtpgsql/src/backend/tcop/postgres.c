					 ...
					 bool aclOverride)
{
	...
	pg_options[TRACE_PARSERSTATS] = 1;
#endif
	...
	querytree_list = parser(query_string, typev, argnames, nargs);
										 ...
										 * of superusers */
{
	...
	querytree_list = pg_parse_and_rewrite(query_string, NULL,NULL, 0,
										  aclOverride);


/*  this should be moved here b/c the query snapshot should be consistent 
	for each issue of a query  MKS  10.6.2003 */
	...
	while ((flag = getopt(argc, argv,
		...
		switch (flag)
		{
			...
			case 'f':
				...
				{
					case 's':	/* seqscan */
						GetCostInfo()->enable_seqscan = false;
						break;
					case 'i':	/* indexscan */
						GetCostInfo()->enable_indexscan = false;
						break;
					case 't':	/* tidscan */
						GetCostInfo()->enable_tidscan = false;
						break;
					case 'n':	/* nestloop */
						GetCostInfo()->enable_nestloop = false;
						break;
					case 'm':	/* mergejoin */
						GetCostInfo()->enable_mergejoin = false;
						break;
					case 'h':	/* hashjoin */
						GetCostInfo()->enable_hashjoin = false;
						break;
					default:
						errs++;
				}
...
/*	pqsignal(SIGCHLD, SIG_IGN); *//* ignored, sent by LockOwners */
	pqsignal(SIGCHLD, SIG_DFL);   /*  changed to allow popen and pclose work properly  */
...
*/
	...
	if (IsUnderPostmaster)
	{
		switch (MyProcPort->raddr.sa.sa_family)
		{
			...
			case AF_INET:
				remote_info = remote_host = os_malloc(48);
				...
				if (ShowPortNumber)
				{
					remote_info = os_malloc(strlen(remote_host) + 6);
	...
	{
		MemoryContext oldcontext = MemoryContextSwitchTo(MemoryContextGetTopContext());
	...
	if (!IsUnderPostmaster)
	{
		...
		puts("$Revision: 1.2 $ $Date: 2007/03/20 03:07:41 $\n");
	}
