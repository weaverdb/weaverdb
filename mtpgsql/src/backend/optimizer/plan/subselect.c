 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/plan/subselect.c,v 1.1.1.1 2006/08/12 00:20:52 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
new_param(Var *var, Index varlevel)
{
	...
	OptimizerGlobals* env = GetOptimizerInfo();

	paramVar->varlevelsup = varlevel;

	env->PlannerParamVar = lappend(env->PlannerParamVar, paramVar);

	return length(env->PlannerParamVar) - 1;
}
...
replace_var(Var *var)
{
	...
	Assert(var->varlevelsup > 0 && var->varlevelsup < env->PlannerQueryLevel);
	varlevel = env->PlannerQueryLevel - var->varlevelsup;
	 ...
	 */
	i = 0;
	foreach(ppv, env->PlannerParamVar)
	{
		Var		   *pvar = lfirst(ppv);
...
make_subplan(SubLink *slink)
{
	...
	List	   *saved_ip = env->PlannerInitPlan;

	env->PlannerInitPlan = NULL;

	env->PlannerQueryLevel++;		/* we become child */
	...
	plan->initPlan = env->PlannerInitPlan;

	/* Create extParam list as union of InitPlan-s' lists */
	foreach(lst, env->PlannerInitPlan)
	{
		List	   *lp;
	...
	env->PlannerInitPlan = saved_ip;
	env->PlannerQueryLevel--;

	node->plan_id = env->PlannerPlanId++;
	...
	{
		Var		   *var = nth(lfirsti(lst), env->PlannerParamVar);

		/* note varlevelsup is absolute level number */
		if (var->varlevelsup == env->PlannerQueryLevel)
	...
	if (node->parParam == NIL && slink->subLinkType == EXISTS_SUBLINK)
	{
		...
		prm->paramid = (AttrNumber) new_param(var, env->PlannerQueryLevel);
		prm->paramtype = var->vartype;
		pfree(var);				/* var is only needed for new_param */
		node->setParam = lappendi(node->setParam, prm->paramid);
		env->PlannerInitPlan = lappend(env->PlannerInitPlan, node);
		result = (Node *) prm;
	}
...
finalize_primnode(Node *node, finalize_primnode_results *results)
{
	...
	if (is_subplan(node))
	{
		...
		foreach(lst, subplan->plan->extParam)
		{
			...
			Var		   *var = nth(paramid, env->PlannerParamVar);
...
/* don't think this is possible in current implementation  */
			finalize_primnode((Node *) ((DelegatedIndexScan *) plan)->indxqual,
							  &results);
	...
	foreach(lst, results.paramids)
	{
		...
		else if (var->varlevelsup > env->PlannerQueryLevel)
