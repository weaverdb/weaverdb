 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/plan/createplan.c,v 1.1.1.1 2006/08/12 00:20:51 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <sys/types.h>
...
static SeqScan *create_seqscan_node(Path *best_path, List *tlist,
					List *scan_clauses,bool delegated);
static Scan *create_indexscan_node(Query *root, IndexPath *best_path,
					  List *tlist, List *scan_clauses, bool delegated);
...
static DelegatedSeqScan    * make_delegated_seqscan(List *qptlist,
			 List *qpqual,
			 Index scanrelid);
...
create_scan_node(Query *root, Path *best_path, List *tlist)
{
	...
	{
		case T_SeqScan:
			node = (Scan *) create_seqscan_node(best_path, tlist, scan_clauses,false);
			break;

		case T_DelegatedSeqScan:
			node = (Scan *) create_seqscan_node(best_path, tlist, scan_clauses,true);
			break;
 ...
 */
static SeqScan *
create_seqscan_node(Path *best_path, List *tlist, List *scan_clauses,bool delegated)
{
	SeqScan    *scan_node;
	Index		scan_relid;
	...
	scan_node = (Scan*)make_delegated_seqscan(tlist,
							 scan_clauses,
							 scan_relid);
        }
					  ...
					  List *scan_clauses,bool delegated)
{
	...
	fixed_indxqual = fix_indxqual_references(indxqual, best_path);
        ...
        if ( !delegated ) {
            ...
            scan_node = (Scan*)make_indexscan(tlist,
                                                               qpqual,
                                                               baserelid,
                                                               best_path->indexid,
                                                               fixed_indxqual,
            ...
            scan_node = (Scan*)make_delegated_indexscan(tlist,
                                                               qpqual,
                                                               baserelid,
                                                               lfirsti(best_path->indexid),
                                                               fixed_indxqual,
                                                               indxqual,
                                                               best_path->indexscandir);
        }
                                                           
	copy_path_costsize(&scan_node->plan, &best_path->path);
	/* use the indexscan-specific rows estimate, not the parent rel's */
        scan_node->plan.plan_rows = best_path->rows;
...
static DelegatedSeqScan    *
make_delegated_seqscan(List *qptlist,
			 List *qpqual,
			 Index scanrelid)
{
        DelegatedSeqScan    *node = makeNode(DelegatedSeqScan);
	Plan	   *plan = &node->scan.plan;
...
static DelegatedIndexScan *
make_delegated_indexscan(List *qptlist,
			   List *qpqual,
			   Index scanrelid,
			   Oid indxid,
			   List *indxqual,
			   ...
			   ScanDirection indexscandir)
{
	DelegatedIndexScan  *node = makeNode(DelegatedIndexScan);
	Plan	   *plan = &node->scan.plan;
...
make_agg(List *tlist, List *qual, Plan *lefttree)
{
	 ...
	 * tuple.
	 */
	plan->total_cost += GetCostInfo()->cpu_operator_cost * plan->plan_rows *
		   ...
		   Plan *lefttree)
{
	 ...
	 */
	plan->total_cost += GetCostInfo()->cpu_operator_cost * plan->plan_rows * ngrp;
...
make_unique(List *tlist, Plan *lefttree, List *distinctList)
{
	 ...
	 * assume all columns get compared at most of the tuples.
	 */
	plan->total_cost += GetCostInfo()->cpu_operator_cost * plan->plan_rows * numCols;
