 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/plan/planner.c,v 1.1.1.1 2006/08/12 00:20:52 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <sys/types.h>
...
planner(Query *parse)
{
	...
	env->PlannerQueryLevel = 1;
	env->PlannerInitPlan = NULL;
	env->PlannerParamVar = NULL;
	env->PlannerPlanId = 0;
	...
	Assert(env->PlannerQueryLevel == 1);

	/* if top-level query had subqueries, do housekeeping for them */
	if (env->PlannerPlanId > 0)
	{
		(void) SS_finalize_plan(result_plan);
		result_plan->initPlan = env->PlannerInitPlan;
	}

	/* executor wants to know total number of Params used overall */
	result_plan->nParamExec = length(env->PlannerParamVar);
...
subquery_planner(Query *parse, double tuple_fraction)
{
	...
	if (GetOptimizerInfo()->PlannerQueryLevel > 1)
	{
		parse->targetList = (List *)
