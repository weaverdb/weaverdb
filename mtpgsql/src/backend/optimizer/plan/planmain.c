...
static SectionId optimizer_id = SECTIONID("OPTI");

#ifdef TLS
TLS OptimizerGlobals* optimizer_globals;
#else
#define optimizer_globals GetEnv()->optimizer_globals
#endif

static Plan    *
subplanner(Query * root, List * flat_tlist, List * qual,
	      ...
	      double tuple_fraction)
{
	...
	if (root->rtable == NIL) {
		/*
		 * If it's not a select, it should have had a target
		 * relation...
		 */
	 ...
	 * toplevel "Result" node, where they will gate execution of the
	 * whole plan (the Result will not invoke its descendant plan unless
	 * the quals are true).  Note that any *really* non-variable quals
	 * will have been optimized away by eval_const_expressions().  What
	 * we're mostly interested in here is quals that depend only on
	 * outer-level vars, although if the qual reduces to "WHERE FALSE"
	 * this path will also be taken.
	 */
	...
	if (constant_qual) {
		...
		/*
		 * Replace the toplevel plan node's flattened target list
		 * with the targetlist given by my caller, so that
 ...
 * subplanner
 * 
 * Subplanner creates an entire plan consisting of joins and scans for
 * processing a single level of attributes.
 * 
 * flat_tlist is the flattened target list qual is the qualification to be
 * satisfied tuple_fraction is the fraction of tuples we expect will be
 * retrieved
 * 
	   ...
	   double tuple_fraction)
{
	...
	if (!final_rel) {
		...
		/*
		 * We expect to end up here for a trivial INSERT ... VALUES
		 * query (which will have a target relation, so it gets past
		 * query_planner's check for empty range table; but the
		 * target rel is unreferenced and not marked inJoinSet, so we
		 * find there is nothing to join).
		 * 
		 * It's also possible to get here if the query was rewritten by
		 * the rule processor (creating rangetable entries not marked
		 * inJoinSet) but the rules either did nothing or were
		 * simplified to nothing by constant-expression folding.  So,
		 * don't complain.
		 */
		root->query_pathkeys = NIL;	/* signal unordered result */
	 ...
	 * already appropriately ordered, we use the cheapest path found
	 * above.
	 */
	 ...
	 * let the caller do that.	union_planner has to be able to add a
...
OptimizerGlobals *
GetOptimizerInfo(void)
{
	OptimizerGlobals *info = optimizer_globals;
	if (info == NULL) {
		info = AllocateEnvSpace(optimizer_id, sizeof(OptimizerGlobals));
		memset(info, 0x00, sizeof(OptimizerGlobals));
                optimizer_globals = info;
	}
	return info;
}
