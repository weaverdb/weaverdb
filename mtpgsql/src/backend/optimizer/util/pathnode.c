 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/util/pathnode.c,v 1.1.1.1 2006/08/12 00:20:55 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
add_path(RelOptInfo *parent_rel, Path *new_path)
{
	...
	List	   *p1;
        ...
        DTRACE_PROBE3(mtpg,showcosts,new_path->pathtype,&new_path->startup_cost,&new_path->total_cost);
	/*
 ...
 */
Path *
create_delegated_seqscan_path(RelOptInfo *rel)
{
/*  same as seqscan, just delegated to sub-thread */
	Path	   *pathnode = create_seqscan_path(rel);

	pathnode->pathtype = T_DelegatedSeqScan;

	cost_delegatedseqscan(pathnode, rel);
...
index_path(Query *root,
	...
	pathnode->path.pathkeys = build_index_pathkeys(root, rel, index, indexscandir);
...
create_index_path(Query *root,
				  ...
				  List *restriction_clauses,
				  ScanDirection indexscandir) {
       IndexPath * pathnode = index_path(root,rel,index,restriction_clauses,indexscandir);

	pathnode->path.pathtype = T_IndexScan;
	cost_index(&pathnode->path, root, rel, index, lfirst(pathnode->indexqual), false);
 ...
 */
IndexPath  *
create_delegated_index_path(Query *root,
				  RelOptInfo *rel,
				  IndexOptInfo *index,
				  List *restriction_clauses,
				  ScanDirection indexscandir)
{
/*  same as an index scan, just delegated out to sub-thread */
        IndexPath*  pathnode = index_path(root,rel,index,restriction_clauses,indexscandir);

	pathnode->path.pathtype = T_DelegatedIndexScan;

	cost_delegated_index(&pathnode->path, root, rel, index, lfirst(pathnode->indexqual), false);
