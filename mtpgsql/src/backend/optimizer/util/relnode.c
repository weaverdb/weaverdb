 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/util/relnode.c,v 1.1.1.1 2006/08/12 00:20:55 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
get_base_rel(Query *root, int relid)
{
	...
	rel->joininfo = NIL;
	rel->innerjoin = NIL;
	rel->min_tuple_size = 0;
	rel->max_tuple_size = 0;
	rel->ave_tuple_size = 0;
	if (relid < 0)
	{
	...
	else
	{
		...
		relation_info(root, relid,
					  &rel->indexed, &rel->pages, &rel->tuples, 
						&rel->min_tuple_size,&rel->max_tuple_size,&rel->ave_tuple_size
						);
	}
