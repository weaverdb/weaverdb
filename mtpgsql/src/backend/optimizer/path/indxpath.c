 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/path/indxpath.c,v 1.1.1.1 2006/08/12 00:20:47 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*  try basic delegated index scans  */			
...
clause_pred_clause_test(Expr *predicate, Node *clause)
{
	...
	scan = heap_beginscan(relation, SnapshotNow, 3, entry);
	tuple = heap_getnext(scan);
	if (!HeapTupleIsValid(tuple))
	{
