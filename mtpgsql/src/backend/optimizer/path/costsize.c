 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/path/costsize.c,v 1.1.1.1 2006/08/12 00:20:46 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
cost_seqscan(Path *path, RelOptInfo *baserel)
{
	...
	Cost		run_cost = 0;
	Cost		cpu_per_tuple;
	CostInfo* 	costinfo = GetCostInfo();

	/* Should only be applied to base relations */
	Assert(length(baserel->relids) == 1);

	if (!costinfo->enable_seqscan)
		startup_cost += costinfo->disable_cost;
...
/*
 * cost_delegatedseqscan
 *	  same as sequencial scan but on heavily threaded systems 
 *      like Sun Ultra T1 or multiprocessor systems, it may be faster
 *      to divide up tasks.  Delegated sequantial scans has one thread
 *      that scans the relation for snapshot valid tuples and faults in pages
 *
 *      the main thread evaluates the tuple and handles all communication with the 
 *      outside world.
 */
void
cost_delegatedseqscan(Path *path, RelOptInfo *baserel)
{
	Cost		startup_cost = 0;
	Cost		run_cost = 0;
	Cost		cpu_per_tuple;
	...
	if (!costinfo->enable_seqscan || !costinfo->enable_delegatedseqscan)
		startup_cost += costinfo->disable_cost;

        if ( !IsDolConnectionAvailable() ) {
            startup_cost += costinfo->thread_startup_cost;
        } else {
            startup_cost += costinfo->delegation_startup_cost;
        }
	...
	/* CPU costs */
	cpu_per_tuple = costinfo->cpu_delegated_tuple_cost + baserel->baserestrictcost;
	run_cost += cpu_per_tuple * baserel->tuples;
 ...
 */
static Cost
cost_nonsequential_access(double relpages,bool delegated)
{
        
	double		relsize;
        CostInfo*  costinfo = GetCostInfo();

        double random_page = ( delegated ) ? ( costinfo->delegated_random_page_cost ) : costinfo->random_page_cost;
	/* don't crash on bad input data */
        
	if (relpages <= 0.0 || costinfo->effective_cache_size <= 0.0)
		return random_page;

	relsize = relpages / costinfo->effective_cache_size;

	if (relsize >= 1.0)
		return random_page * (1.0 - 0.5 / relsize);
	else
		return 1.0 + (random_page * 0.5 - 1.0) * relsize * relsize;
}
		   ...
		   bool is_injoin)
{
	...
	if (!costinfo->enable_indexscan && !is_injoin)
		startup_cost += costinfo->disable_cost;
 ...
 */
void
cost_delegated_index(Path *path, Query *root,
		   RelOptInfo *baserel,
		   IndexOptInfo *index,
		   List *indexQuals,
		   bool is_injoin)
...
CostInfo*  costinfo = GetCostInfo();
	...
	if ((!costinfo->enable_indexscan || !costinfo->enable_delegatedindexscan) && !is_injoin)
		startup_cost += costinfo->disable_cost;
	...
	/* disk costs for main table */
	run_cost += (pages_fetched * cost_nonsequential_access(baserel->pages,true));


	/* CPU costs */
	cpu_per_tuple = costinfo->cpu_delegated_index_tuple_cost + baserel->baserestrictcost;
...
CostInfo*  costinfo = GetCostInfo();
	...
	if (!costinfo->enable_tidscan)
		startup_cost += costinfo->disable_cost;

	/* disk costs --- assume each tuple on a different page */
	run_cost += costinfo->random_page_cost * ntuples;
...
CostInfo*  costinfo = GetCostInfo();
	 ...
	 * comparisons
	 */
	startup_cost += 2.0 * costinfo->cpu_operator_cost * tuples * LOG2(tuples);
	...
	if (nbytes > sortmembytes)
	{
		...
		startup_cost += npageaccesses *
			(1.0 + cost_nonsequential_access(npages,false)) * 0.5;
	}
...
CostInfo*  costinfo = GetCostInfo();
	...
	/* CPU costs */
	cpu_per_tuple = costinfo->cpu_tuple_cost + cost_qual_eval(restrictlist);
	run_cost += cpu_per_tuple * ntuples;
...
CostInfo*  costinfo = GetCostInfo();
	...
	startup_cost += costinfo->cpu_operator_cost * inner_path->parent->rows;
	run_cost += costinfo->cpu_operator_cost * outer_path->parent->rows;
...
CostInfo*  costinfo = GetCostInfo();
	...
	if (IsA(node, Expr))
	{
		...
		switch (expr->opType)
		{
			case OP_EXPR:
			case FUNC_EXPR:
				*total += costinfo->cpu_operator_cost;
				break;
			case OR_EXPR:
			case AND_EXPR:
			case NOT_EXPR:
...
set_rel_width(Query *root, RelOptInfo *rel)
{
	...
	if ( rel->ave_tuple_size != 0 ) {
		rel->width = rel->ave_tuple_size;
	} else {
