 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/optimizer/path/allpaths.c,v 1.1.1.1 2006/08/12 00:20:45 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
set_base_rel_pathlist(Query *root)
{
	...
	foreach(rellist, root->base_rel_list)
	{
		...
		/* Consider delegated sequential scan */
		add_path(rel, create_delegated_seqscan_path(rel));
		/* Consider TID scans */
