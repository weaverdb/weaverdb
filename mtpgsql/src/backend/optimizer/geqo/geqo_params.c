...
*
* $Id: geqo_params.c,v 1.1.1.1 2006/08/12 00:20:45 synmscott Exp $
*
*-------------------------------------------------------------------------
*/
...
geqo_params(int string_length)
{
	...
	if (file)
	{
		...
		while ((i = next_token(file, buf, sizeof(buf))) != EOF)
		{
			...
			if (strcmp(buf, POOL_TAG) == 0)
			{
				...
				{
					if (sscanf(buf, "%d", &env->PoolSize) == 1)
						pool_size = 1;
				}
			...
			else if (strcmp(buf, TRIAL_TAG) == 0)
			{
				...
				if (i != EOF)
				{
					if (sscanf(buf, "%d", &env->Generations) == 1)
						number_trials = 1;
				}
			...
			else if (strcmp(buf, RAND_TAG) == 0)
			{
				...
				if (i != EOF)
				{
					if (sscanf(buf, "%ld", &env->RandomSeed) == 1)
						random_seed = 1;
				}
			...
			else if (strcmp(buf, BIAS_TAG) == 0)
			{
				...
				if (i != EOF)
				{
					if (sscanf(buf, "%lf", &env->SelectionBias) == 1)
						selection_bias = 1;
				}
	...
	if (!(pool_size))
	{
		env->PoolSize = gimme_pool_size(string_length);

		elog(DEBUG, "geqo_params: no pool size specified;\nusing computed value of %d", env->PoolSize);
	}
	...
	if (!(number_trials))
	{
		env->Generations = gimme_number_generations(env->PoolSize, effort);

		elog(DEBUG, "geqo_params: no number of trials specified;\nusing computed value of %d", env->Generations);
	...
	if (!(random_seed))
	{
		env->RandomSeed = (long) time(NULL);

		elog(DEBUG, "geqo_params: no random seed specified;\nusing computed value of %ld", env->RandomSeed);
	}

	/* SelectionBias: */
	if (!(selection_bias))
	{
		env->SelectionBias = SELECTION_BIAS;

		elog(DEBUG, "geqo_params: no selection bias specified;\nusing default value of %f", env->SelectionBias);
	}
