 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/hash/hashpage.c,v 1.1.1.1 2006/08/12 00:19:58 synmscott Exp $
 *
 * NOTES
...
#define USELOCKING		(!GetIndexGlobals()->BuildingHash && !IsInitProcessingMode())
...
_hash_getbuf(Relation rel, BlockNumber blkno, int access)
{
	...
	buf = ReadBuffer(rel, blkno);
        if (!BufferIsValid(buf) ) 
                    elog(ERROR,"bad buffer read while scanning hash %s",RelationGetRelationName(rel));
