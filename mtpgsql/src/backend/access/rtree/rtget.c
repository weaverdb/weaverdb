 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/rtree/rtget.c,v 1.1.1.1 2006/08/12 00:20:04 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
rtfirst(IndexScanDesc s, ScanDirection dir)
{
	...
	b = ReadBuffer(s->relation, P_ROOT);
                if (!BufferIsValid(b) ) elog(ERROR,"bad buffer read in rtree get");
	p = BufferGetPage(b);
