 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/rtree/rtree.c,v 1.1.1.1 2006/08/12 00:20:04 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
		...
		PredInfo *predInfo)
{
	...
	if (oldPred == NULL)
	{
		buffer = ReadBuffer(index, P_NEW);
                if (!BufferIsValid(buffer) ) elog(ERROR,"bad buffer read in rtree");
	...
	if (pred != NULL || oldPred != NULL)
	{
		...
		FillDummyExprContext(econtext, slot, hd);
	}
	else
	{
		econtext = NULL;
	...
	while (HeapTupleIsValid(htup = heap_getnext(scan)))
	{
		nh++;
	...
	if (IsNormalProcessingMode())
	{
		...
		index_close(index);
		UpdateStats(hrelid, nh);
		UpdateStats(irelid, ni);
...
rtdoinsert(Relation r, IndexTuple itup, RTSTATE *rtstate)
{
	...
	if (nospace(page, itup))
	{
		...
		freestack(stack);
		WriteBuffer(r, buffer);	/* don't forget to release buffer! */
		return res;
	}
		  ...
		  RTSTATE *rtstate)
{
	...
	b = ReadBuffer(r, stk->rts_blk);
                if (!BufferIsValid(b) ) elog(ERROR,"bad buffer read in rtree");
	p = BufferGetPage(b);
		...
		RTSTATE *rtstate)
{
	...
	if (BufferGetBlockNumber(buffer) == P_ROOT)
	{
		leftbuf = ReadBuffer(r, P_NEW);
                if (!BufferIsValid(leftbuf) ) elog(ERROR,"bad buffer read in rtree");
	...
	{
		leftbuf = buffer;
		IncrBufferRefCount(r,buffer);
		lbknum = BufferGetBlockNumber(buffer);
		left = (Page) PageGetTempPage(p, sizeof(RTreePageOpaqueData));
	}

	rightbuf = ReadBuffer(r, P_NEW);
                if (!BufferIsValid(rightbuf) ) elog(ERROR,"bad buffer read in rtree");
	...
	WriteBuffer(r, leftbuf);
	WriteBuffer(r, rightbuf);
			...
			RTSTATE *rtstate)
{
	...
	if (nospace(p, rtup))
	{
		...
		WriteBuffer(r, b);			/* don't forget to release buffer!  -
								 * 01/31/94 */
		pfree(res);
	}
	else
...
rtdelete(Relation r, ItemPointer tid)
{
	...
	buf = ReadBuffer(r, blkno);
                if (!BufferIsValid(buf) ) elog(ERROR,"bad buffer read in rtree");
