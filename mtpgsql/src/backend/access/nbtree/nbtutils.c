 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/nbtree/nbtutils.c,v 1.1.1.1 2006/08/12 00:20:04 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
{
	Relation	relation = scan->relation;
	...
	for (i = 0;; cur++, i++)
	{
		...
		if (i == numberOfKeys || cur->sk_attno != attno)
		{
			...
			if (init[BTEqualStrategyNumber - 1])
			{
				...
				for (j = BTMaxStrategyNumber; --j >= 0;)
				{
					if (!init[j] ||
						...
						continue;
					chk = &xform[j];
					test = DatumGetChar(fmgr(chk->sk_procedure,
				...
				&& init[BTLessEqualStrategyNumber - 1])
			{
				...
				test = DatumGetChar(fmgr(le->sk_procedure,
										lt->sk_argument,
										le->sk_argument));
				if (test)
					init[BTLessEqualStrategyNumber - 1] = false;
				...
				&& init[BTGreaterEqualStrategyNumber - 1])
			{
				...
				test = DatumGetChar(fmgr(ge->sk_procedure,
										gt->sk_argument,
										ge->sk_argument));
				if (test)
					init[BTGreaterEqualStrategyNumber - 1] = false;
		...
		if (init[j])
		{
			...
			test = DatumGetChar((*fmgr_faddr(&cur->sk_func)) ( cur->sk_argument,
								 xform[j].sk_argument));
			if (test)
	...
	*continuescan = true;
	...
	while (keysz > 0)
	{
		...
		if (key->sk_flags & SK_COMMUTE)
			test = DatumGetChar((*fmgr_faddr(&key->sk_func))(key->sk_argument, datum));
		else
			test = DatumGetChar((*fmgr_faddr(&key->sk_func))(datum, key->sk_argument));
