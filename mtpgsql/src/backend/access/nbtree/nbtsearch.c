 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/nbtree/nbtsearch.c,v 1.3 2007/04/22 23:16:49 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
		   ...
		   Buffer *bufP, int access)
{
	...
	for (;;)
	{
		...
		if ( new_stack == NULL ) {
			elog(FATAL,"Out of Memory Error in _bt_search");
		}
			...
			OffsetNumber offnum)
{
	...
	for (i = 0; i < keysz; i++)
	{
		...
		{
			result = DatumGetInt32((*fmgr_faddr(&entry->sk_func)) (
												 entry->sk_argument,
												 datum));
		}
	...
	do
	{
		...
		if (_bt_checkkeys(scan, itup, dir, &continuescan))
		{
			...
			/* tuple passes all scan key conditions, so return it */
                        ...
                        if ( changed ) WriteNoReleaseBuffer(scan->relation, buf);
...
_bt_first(IndexScanDesc scan, ScanDirection dir)
{
	...
	switch (strat_total)
	{
		case BTLessStrategyNumber:
			 ...
			 */
			if (!_bt_step(scan, BackwardScanDirection))
			{
                            pfree(scankeys);
                            return false;
                        } else {
	...
	{
			/* tuple passes all scan key conditions, so return it */
			/* but first make sure that the transaction that added 
				this index item didn't crash,  This prevents an occurance
				where the index page gets written to disk but the heap 
				page associated with it does not before crash  MKS  12.2.2003
			*/
 ...
 */
bool
_bt_step(IndexScanDesc scan, ScanDirection dir)
{
	Relation	rel = scan->relation;
	 ...
	 */
	offnum = ItemPointerGetOffsetUnchecked(current);
            ...
            else
            {
                ...
                for (;;)
                {
                    ...
                    if (P_RIGHTMOST(opaque))
                    {
                        _bt_relbuf(rel, buf);
                        ItemPointerSetInvalid(current);
                        buf = so->btso_curbuf = InvalidBuffer;
                        return false;
                    }
                    /* step right one page */
