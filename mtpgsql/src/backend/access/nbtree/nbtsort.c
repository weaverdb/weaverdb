...
/*
 * Status record for a btree page being built.	We have one of these for each
 * active tree level.
 * 
 * The reason we need to store a copy of the minimum key is that we'll need to
 * propagate it to the parent node when this page is linked into its parent.
 * However, if the page is not a leaf page, the first entry on the page
 * doesn't need to contain a key, so we will not have stored the key itself
 * on the page.  (You might think we could skip copying the minimum key on
 * leaf pages, but actually we must have a writable copy anyway because we'll
 * poke the page's address into it before passing it up to the parent...)
 */
typedef struct BTPageState {
	Buffer          btps_buf;	/* current buffer & page */
	Page            btps_page;
	BTItem          btps_minkey;	/* copy of minimum key (first item)
					 * on page */
...
static void 
_bt_sortaddtup(Page page, Size itemsize,
...
_bt_spool(BTItem btitem, BTSpool * btspool)
{
	...
	tuplesort_puttuple(btspool->sortstate, (void *) &btitem->bti_itup);
}

/*
 * given a spool loaded by successive calls to _bt_spool, create an entire
 * btree.
 */
void
...
void
_bt_spoolmerge(BTSpool * dest, BTSpool * src)
{
	bool            should_free;
	IndexTuple      it;
	BTItem          bti;
	if (src->isunique) {
		tuplesort_performsort(src->sortstate);
	}
	it = (IndexTuple) tuplesort_getindextuple(src->sortstate, true, &should_free);
	while (it) {
		bti = _bt_formitem(it);
		_bt_spool(bti, dest);
		if (should_free) {
...
/*
 * allocate and initialize a new BTPageState.  the returned structure is
 * suitable for immediate use by _bt_buildadd.
 */
static BTPageState *
...
/*
 * slide an array of ItemIds back one slot (from P_FIRSTKEY to P_HIKEY,
 * overwriting P_HIKEY).  we need to do this when we discover that we have
 * built an ItemId array in what has turned out to be a P_RIGHTMOST page.
 */
static void
 ...
 * 
 * The main difference between this routine and a bare PageAddItem call is that
 * this code knows that the leftmost data item on a non-leaf btree page
 * doesn't need to have a key.  Therefore, it strips such items down to just
 * the item header.
 * 
 * This is almost like nbtinsert.c's _bt_pgaddtup(), but we can't use that
 * because it assumes that P_RIGHTMOST() will return the correct answer for
 * the page.  Here, we don't know yet if the page will be rightmost.  Offset
 * P_FIRSTKEY is always the first data key.
 */
static void
...
_bt_buildadd(Relation index, BTPageState * state, BTItem bti)
{
	 ...
	 * (Eventually, we ought to try to apply TOAST methods if not.) We
	 * actually need to be able to fit three items on every page, so
	 * restrict any one item to 1/3 the per-page available space. Note
	 * that at this point, btisz doesn't include the ItemId.
	 * 
	...
	if (pgspc < btisz || pgspc < state->btps_full) {
		/*
		 * Item won't fit on this page, or we feel the page is full
		 * enough already.  Finish off the page and write it out.
		 */
		Buffer          obuf = nbuf;
		Page            opage = npage;
		ItemId          ii;
		...
		/*
		 * We copy the last item on the page into the new page, and
		 * then rearrange the old page so that the 'last item'
		 * becomes its high key rather than a true data item.  There
		 * had better be at least two items on the page already, else
		 * the page would be empty of useful data.  (Hence, we must
		 * allow pages to be packed at least 2/3rds full; the 70%
		 * figure used above is close to minimum.)
		 */
		...
		*hii = *ii;
		...
		/*
		 * Link the old buffer into its parent, using its minimum
		 * key. If we don't have a parent, we have to create one;
		...
		/*
		 * Save a copy of the minimum key for the new page.  We have
		 * to copy it off the old page, not the new one, in case we
		 * are not at leaf level.
		 */
		state->btps_minkey = _bt_formitem(&(obti->bti_itup));

		/*
		 * Set the sibling links for both pages, and parent links
		 * too.
		 * 
		 * It's not necessary to set the parent link at all, because
		 * it's only used for handling concurrent root splits, but we
		 * may as well do it as a debugging aid.  Note we set new
		 * page's link as well as old's, because if the new page
		 * turns out to be the last of the level, _bt_uppershutdown
		 * won't change it.  The links may be out of date by the time
		 * the build finishes, but that's OK; they need only point to
		 * a left-sibling of the true parent.  See the README file
		 * for more info.
		 */
		{
		...
		/*
		 * Write out the old page.	We never want to see it
		 * again, so we can give up our lock (if we had one; most
		 * likely BuildingBtree is set, so we aren't locking).
		 */
...
_bt_uppershutdown(Relation index, BTPageState * state)
{
	...
	for (s = state; s != (BTPageState *) NULL; s = s->btps_next) {
		 ...
		 * 
		 * If we're at the top, it's the root, so attach it to the
		 * metapage. Otherwise, add an entry for it to its parent
		 * using its minimum key.  This may cause the last page of
		 * the parent level to split, but that's not a problem -- we
		...
		/*
		 * This is the rightmost page, so the ItemId array needs to
		 * be slid back one slot.	Then we can dump out the
		 * page.
		 */
...
/*
 * Read tuples in correct sort order from tuplesort, and load them into btree
 * leaves.
 */
static void
