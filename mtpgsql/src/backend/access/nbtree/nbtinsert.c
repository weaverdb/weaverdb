...
/* if this is the poolsweep thread, don't bother.
	we are assuming that it is unique b/c the 
	poolsweep would only move a tuple not change or insert 
	new MKS 11/21/2003

	this may be causing errors, rethink logic
*/
                                 ...
                                 Buffer buf, ScanKey itup_scankey)
{
        ...
        Snapshot Dirty = GetSnapshotDirty(RelationGetSnapshotCxt(rel));
                ...
                if (offset <= maxoff)
                {
                        ...
                        if ( heap_fetch(heapRel, Dirty, &htup, &hbuffer)) {
                               /* it is a duplicate */
                                ...
                                Dirty->xmin : Dirty->xmax;

                               ReleaseBuffer(heapRel, hbuffer);
			   ...
			   OffsetNumber afteritem)
{
	...
	if (PageGetFreeSpace(page) < itemsz)
	{
		...
		else
		{
			...
			ritem = (BTItem) PageGetItem(page,PageGetItemId(page, P_HIKEY));
	...
	for (;;)
	{
			...
			(cidx == 2 && coff[2] == InvalidOffsetNumber))
		{
			...
			for (i = 1; i <= cidx; i++)
			{
				...
				/* insert pointer */
				ritem = (BTItem) PageGetItem(cpage[i - 1],PageGetItemId(cpage[i - 1], P_HIKEY));
				btitem = _bt_formitem(&(ritem->bti_itup));
