 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/common/indextuple.c,v 1.1.1.1 2006/08/12 00:19:55 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
				...
				char *null)
{
	...
	if (size & 0xE000)
		elog(ERROR, "index_formtuple: data takes %d bytes: too big", (uint32)size);
...
/* This is handled in the macro */
		...
		if (attnum == 1)
			return (Datum) HeapFetchAtt(&(att[0]), (char *) tup + data_off);
...
/* This is handled in the macro */
	...
	if (!slow)
	{
		if (att[attnum]->attcacheoff != -1)
		{
			...
			return (Datum) HeapFetchAtt(&(att[0]), (char *) tp);
