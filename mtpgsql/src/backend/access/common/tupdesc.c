 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/access/common/tupdesc.c,v 1.1.1.1 2006/08/12 00:19:56 synmscott Exp $
 *
 * NOTES
...
CreateTupleDesc(int natts, Form_pg_attribute *attrs)
{
	...
	desc->blobatt = 0;
	for(c=0;c<natts;c++) {
		Form_pg_attribute  check = *(attrs + c);
		if ( check->attstorage == 'e' ) {
				desc->blobatt = c+1;
				break;
		}
	}
...
CreateTupleDescCopyConstr(TupleDesc tupdesc)
{
	...
	desc->blobatt = tupdesc->blobatt;
	size = desc->natts * sizeof(Form_pg_attribute);
	if ( size > 0 ) desc->attrs = (Form_pg_attribute *) palloc(size);
	for (i = 0; i < desc->natts; i++)
	{
		desc->attrs[i] = (Form_pg_attribute) palloc(ATTRIBUTE_TUPLE_SIZE);
		memmove(desc->attrs[i],
...
equalTupleDescs(TupleDesc tupdesc1, TupleDesc tupdesc2)
{
	...
	if (tupdesc1->natts != tupdesc2->natts)
		return false;
	if (tupdesc1->blobatt != tupdesc2->blobatt)
		return false;
...
BuildDescForRelation(List *schema, char *relname)
{
	...
	foreach(p, schema)
	{
		...
		if ( desc->attrs[attnum-1]->attstorage == 'e' ) {
			desc->blobatt = attnum;
		}
		/*
