 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/commands/copy.c,v 1.1.1.1 2006/08/12 00:20:19 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
static void CopyFrom(StringInfo attribute_buf,Relation rel, bool binary, bool oids, FILE *fp, char *delim, char *null_print);
...
static char *CopyReadAttribute(StringInfo attrubite_buf,FILE *fp, bool *isnull, char *delim, int *newline, char *null_print);
...
----------------------------------------------------------------------------*/
	...
	{							/* copy from file to database */
		...
		CopyFrom(&attribute_buf,rel, binary, oids, fp, delim, null_print);
	}
	else
...
CopyTo(Relation rel, bool binary, bool oids, FILE *fp, char *delim, char *null_print)
{
	...
	scandesc = heap_beginscan(rel, GetSnapshotQuery(RelationGetSnapshotCxt(rel)), 0, NULL);
	...
	while (HeapTupleIsValid(tuple = heap_getnext(scandesc)))
	{
		if ( CheckForCancel() ) {
		...
		for (i = 0; i < attr_count; i++)
		{
			value = HeapGetAttr(tuple, i + 1, tupDesc, &isnull);
			if (!binary)
			{
#ifdef	_DROP_COLUMN_HACK__
				if (!valid[i])
				...
				if (!isnull)
				{
					string = DatumGetPointer((*fmgr_faddr_3(&out_functions[i]))(value, elements[i], typmod[i]));
			...
			else
			{
				...
				/*
				 * only interesting thing HeapGetAttr tells us in this
...
static void
CopyFrom(StringInfo attribute_buf,Relation rel, bool binary, bool oids, FILE *fp, char *delim, char *null_print)
{
	HeapTuple	tuple;
			...
			for (i = 0; i < n_indices; i++)
			{
				...
				if (VARSIZE(&pgIndexP[i]->indpred) != 0)
				{
					...
					if (econtext == NULL)
					{
						 ...
						 * attribute.  If it did, then HeapGetAttr would
	...
	while (!done)
	{
		...
		if (!binary)
		{
			...
			if (oids)
			{
				string = CopyReadAttribute(attribute_buf,fp, &isnull, delim, &newline, null_print);
				if (string == NULL)
					done = 1;
				else
				{
			...
			for (i = 0; i < attr_count && !done; i++)
			{
				...
				{
					values[i] = PointerGetDatum((*fmgr_faddr_3(&in_functions[i]))(PointerGetDatum(string),
															 elements[i],
															  typmod[i]));
...
GetOutputFunction(Oid type)
{
	...
	elog(ERROR, "GetOutputFunction: Cache lookup of type %lu failed", type);
	return InvalidOid;
}
...
GetInputFunction(Oid type)
{
	...
	elog(ERROR, "GetInputFunction: Cache lookup of type %lu failed", type);
	return InvalidOid;
}
				  ...
				  Relation **index_rels)
{
	...
	scandesc = heap_beginscan(pg_index_rel, SnapshotNow, 0, NULL);
	...
	*n_indices = 0;
	...
	while (HeapTupleIsValid(tuple = heap_getnext(scandesc)))
	{
		...
		index_relation_oid = (Oid) DatumGetInt32(HeapGetAttr(tuple, 2,
													  tupDesc, &isnull));
		if (index_relation_oid == main_relation_oid)
		{
			scan->index_rel_oid = (Oid) DatumGetInt32(HeapGetAttr(tuple,
...
static char *
CopyReadAttribute(StringInfo attribute_buf,FILE *fp, bool *isnull, char *delim, int *newline, char *null_print)
{
	int			c;
...
CountTuples(Relation relation)
{
	...
	scandesc = heap_beginscan(relation, GetSnapshotQuery(RelationGetSnapshotCxt(relation)), 0, NULL);
