 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/commands/indexcmds.c,v 1.1.1.1 2006/08/12 00:20:21 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
			   ...
			   char *accessMethodName, Oid accessMethodId)
{
	...
	scan = heap_beginscan(relation, SnapshotNow, 2, entry);
 ...
 */
void
ReindexIndex(const char *name, bool force /* currently unused */ ,bool exclusive)
{
	HeapTuple	tuple;
	bool		success;
	...
	success = reindex_index(tuple->t_data->t_oid, force);
 ...
 */
void
ReindexTable(const char *name, bool force,bool exclusive)
{
	HeapTuple	tuple;
	bool 		success;
	...
	if ( exclusive ) MasterWriteLock();	
	success = reindex_relation(tuple->t_data->t_oid, force);
	if ( exclusive ) MasterUnLock();
...
void
ReindexDatabase(const char *dbname, bool force, bool all,bool exclusive)
{
	 ...
	 */
	private_context = AllocSetContextCreate(MemoryContextGetEnv()->QueryContext,
											"ReindexDatabase",
