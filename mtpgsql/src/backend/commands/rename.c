		  ...
		  int recurse)
{
	...
	if (recurse)
	{
		...
		foreach(child, children)
		{
			...
			{
				elog(ERROR, "renameatt: can't find catalog entry for inheriting class with oid %lu",
					 childrelid);
			}
...
renamerel(const char *oldrelname, const char *newrelname)
{
	 ...
	 */
	targetrelation = RelationNameGetRelation(oldrelname,GetDatabaseId());
	 ...
	 */
	smgrclose(targetrelation->rd_smgr);
	...
	RelationIdInvalidateRelationCache(reloid,GetDatabaseId());
