 ...
 *
 * $Id: user.c,v 1.1.1.1 2006/08/12 00:20:23 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <sys/types.h>
...
write_password_file(Relation rel)
{
	...
	while (HeapTupleIsValid(tuple = heap_getnext(scan)))
	{
		...
		datum_n = HeapGetAttr(tuple, Anum_pg_shadow_usename, dsc, &null_n);
		if (null_n)
			continue;			/* don't allow empty users */
		datum_p = HeapGetAttr(tuple, Anum_pg_shadow_passwd, dsc, &null_p);
		...
		if (null_p)
			continue;
		datum_v = HeapGetAttr(tuple, Anum_pg_shadow_valuntil, dsc, &null_v);
...
CreateUser(CreateUserStmt *stmt)
{
	...
	while (!user_exists && !sysid_exists && HeapTupleIsValid(tuple = heap_getnext(scan)))
	{
		Datum		datum;
		bool		null;

		datum = HeapGetAttr(tuple, Anum_pg_shadow_usename, pg_shadow_dsc, &null);
		user_exists = datum && !null && (strcmp((char *) datum, stmt->user) == 0);

		datum = HeapGetAttr(tuple, Anum_pg_shadow_usesysid, pg_shadow_dsc, &null);
...
AlterUser(AlterUserStmt *stmt)
{
	...
	/* sysid - leave as is */
	new_record[Anum_pg_shadow_usesysid - 1] = HeapGetAttr(tuple, Anum_pg_shadow_usesysid, pg_shadow_dsc, &null);
	...
	{
		/* don't change */
		new_record[Anum_pg_shadow_usecreatedb - 1] = HeapGetAttr(tuple, Anum_pg_shadow_usecreatedb, pg_shadow_dsc, &null);
	...
	/* trace - leave as is */
	new_record[Anum_pg_shadow_usetrace - 1] = HeapGetAttr(tuple, Anum_pg_shadow_usetrace, pg_shadow_dsc, &null);
	...
	{
		/* don't change */
		new_record[Anum_pg_shadow_usesuper - 1] = HeapGetAttr(tuple, Anum_pg_shadow_usesuper, pg_shadow_dsc, &null);
	...
	{
		/* leave alone */
		new_record[Anum_pg_shadow_usecatupd - 1] = HeapGetAttr(tuple, Anum_pg_shadow_usecatupd, pg_shadow_dsc, &null);
	...
	{
		/* leave as is */
		new_record[Anum_pg_shadow_passwd - 1] = HeapGetAttr(tuple, Anum_pg_shadow_passwd, pg_shadow_dsc, &null);
	...
	{
		/* leave as is */
		new_record[Anum_pg_shadow_valuntil - 1] = HeapGetAttr(tuple, Anum_pg_shadow_valuntil, pg_shadow_dsc, &null);
...
DropUser(DropUserStmt *stmt)
{
	...
	foreach(item, stmt->users)
	{
		...
		usesysid = DatumGetInt32(HeapGetAttr(tuple, Anum_pg_shadow_usesysid, pg_shadow_dsc, &null));
		...
		if (HeapTupleIsValid(tmp_tuple = heap_getnext(scan)))
		{
			datum = HeapGetAttr(tmp_tuple, Anum_pg_database_datname, pg_dsc, &null);
			heap_close(pg_shadow_rel, AccessExclusiveLock);
		 ...
		 */
		heap_delete(pg_shadow_rel, &tuple->t_self, NULL,NULL);
		...
		while (HeapTupleIsValid(tmp_tuple = heap_getnext(scan)))
		{
			AlterGroupStmt ags;

			datum = HeapGetAttr(tmp_tuple, Anum_pg_group_groname, pg_dsc, &null);
...
CreateGroup(CreateGroupStmt *stmt)
{
	...
	while (!group_exists && !sysid_exists && HeapTupleIsValid(tuple = heap_getnext(scan)))
	{
		Datum		datum;
		bool		null;

		datum = HeapGetAttr(tuple, Anum_pg_group_groname, pg_group_dsc, &null);
		group_exists = datum && !null && (strcmp((char *) datum, stmt->name) == 0);

		datum = HeapGetAttr(tuple, Anum_pg_group_grosysid, pg_group_dsc, &null);
...
AlterGroup(AlterGroupStmt *stmt, const char *tag)
{
								 ...
								 * create user */
	{
		...
		HeapTuple	tuple;
		bool		null = false;
		Datum		datum = HeapGetAttr(group_tuple, Anum_pg_group_grolist, pg_group_dsc, &null);
		int			i;
		 ...
		 */
		new_record[Anum_pg_group_groname - 1] = (Datum) (stmt->name);
		new_record[Anum_pg_group_grosysid - 1] = HeapGetAttr(group_tuple, Anum_pg_group_grosysid, pg_group_dsc, &null);
	...
	else if (stmt->action == -1)/* drop users from group */
	{
		...
		datum = HeapGetAttr(group_tuple, Anum_pg_group_grolist, pg_group_dsc, &null);
		if (null)
		{
			if (!is_dropuser)
...
DropGroup(DropGroupStmt *stmt)
{
	...
	while (HeapTupleIsValid(tuple = heap_getnext(scan)))
	{
		...
		{
			gro_exists = true;
			heap_delete(pg_group_rel, &tuple->t_self, NULL,NULL);
		}
	}
