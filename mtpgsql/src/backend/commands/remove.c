 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/commands/remove.c,v 1.1.1.1 2006/08/12 00:20:21 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
			   ...
			   char *typeName2) /* optional second type name */
{
	...
	if (HeapTupleIsValid(tup))
	{
		...
		heap_delete(relation, &tup->t_self, NULL,NULL);
...
RemoveFunction(char *functionName,		/* function name to be removed */
			   int nargs,
			   List *argNameList /* list of TypeNames */,Node * retNode )
{
	Relation	relation;
	HeapTuple	tup;
