				   ...
				   CommandDest dest)
{
	 ...
	 * ----------------
	 */
	MemoryContextGetEnv()->PortalExecutorHeapMemory = (MemoryContext) PortalGetHeapMemory(portal);
...
/* DROP DEFAULT */
	{
		...
		scan = heap_beginscan(attr_rel, SnapshotNow, 3, scankeys);
		AssertState(scan != NULL);

		if (HeapTupleIsValid(tuple = heap_getnext(scan)))
		{
			HeapTuple	newtuple;
...
drop_default(Oid relid, int16 attnum)
{
	...
	if (HeapTupleIsValid(tuple = heap_getnext(scan)))
		heap_delete(attrdef_rel, &tuple->t_self, NULL, NULL);
...
systable_beginscan(Relation rel, const char *indexRelname, int nkeys, ScanKey entry)
{
	...
	if (hasindex)
	{
		...
		sysscan->scan = heap_beginscan(rel, SnapshotNow, nkeys, entry);
	return (void *) sysscan;
}
static void
...
systable_getnext(void *scan)
{
	...
	if (sysscan->irel)
	{
		...
		while (index_getnext(sysscan->iscan, ForwardScanDirection))
		{
			sysscan->tuple.t_self = sysscan->iscan->xs_ctup.t_self;
		...
		htup = heap_getnext(sysscan->scan);
	return htup;
}
...
RemoveColumnReferences(Oid reloid, int attnum, bool checkonly, HeapTuple reltup)
{
	...
	while (HeapTupleIsValid(htup = systable_getnext(sysscan)))
	{
		...
		if (find_attribute_in_node(node, attnum))
		{
			...
			{
				heap_delete(rcrel, &htup->t_self, NULL, NULL);
				pgcform->relchecks--;
			}
		}
	}
					 ...
					 int behavior)
{
	...
	switch (heap_mark4update(rel, &buffer, &classtuple))
	{
