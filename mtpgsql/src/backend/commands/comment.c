...
CreateComments(Oid oid, char *comment)
{
	...
	scan = heap_beginscan(description, SnapshotNow, 1, &entry);
	searchtuple = heap_getnext(scan);
	...
	if (HeapTupleIsValid(searchtuple))
	{
		...
		if ((comment == NULL) || (strlen(comment) == 0))
			heap_delete(description, &searchtuple->t_self, NULL, NULL);
		else
		{
...
CommentTrigger(char *trigger, char *relname, char *comment)
{
	...
	scan = heap_beginscan(pg_trigger, SnapshotNow, 1, &entry);
	triggertuple = heap_getnext(scan);
