 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/parser/parse_func.c,v 1.1.1.1 2006/08/12 00:21:11 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <signal.h>
				   ...
				   CandidateList *candidates)
{
	...
	*candidates = NULL;
	...
	while (HeapTupleIsValid(tup = heap_getnext(pg_aggregate_scan)))
	{
				  ...
				  int *curr_resno, int precedence)
{
	...
	else
	{
		...
		exists = func_get_detail(funcname, nargs, oid_array, &funclang,&funcid,
	...
	make_arguments(pstate, nargs, fargs, oid_array, true_oid_array);
			...
			/* got it */
	if ( funclang == JAVAlanguageId ) {
		Java* jc = makeNode(Java);
...
find_inheritors(Oid relid, Oid **supervec)
{
	...
	do
	{
		...
		while (HeapTupleIsValid(inhtup = heap_getnext(inhscan)))
		{
