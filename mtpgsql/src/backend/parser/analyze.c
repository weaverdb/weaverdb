 ...
 *
 *	$Id: analyze.c,v 1.1.1.1 2006/08/12 00:20:57 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
typedef struct analyze_globals {
	List*			extras_before;
	List*			extras_after;
} AnalyzeGlobals;

static SectionId analyze_id = SECTIONID("PLAZ");

#ifdef TLS
TLS AnalyzeGlobals* parseranalyze_globals = NULL;
#else
#define parseranalyze_globals GetEnv()->parseranalyze_globals
#endif

static AnalyzeGlobals* GetAnalyzeInfo(void);
...
parse_analyze(List *pl, ParseState *parentParseState)
{
	...
	while (pl != NIL)
	{
		env->extras_before = env->extras_after = NIL;
		...
		while (env->extras_before != NIL)
		{
			result = lappend(result,
						   transformStmt(pstate, lfirst(env->extras_before)));
			if (pstate->p_target_relation != NULL)
				heap_close(pstate->p_target_relation, AccessShareLock);
			pstate->p_target_relation = NULL;
			pstate->p_target_rangetblentry = NULL;
			env->extras_before = lnext(env->extras_before);
		}

		result = lappend(result, parsetree);

		while (env->extras_after != NIL)
		{
			result = lappend(result,
							 transformStmt(pstate, lfirst(env->extras_after)));
			if (pstate->p_target_relation != NULL)
				heap_close(pstate->p_target_relation, AccessShareLock);
			pstate->p_target_relation = NULL;
			pstate->p_target_rangetblentry = NULL;
			env->extras_after = lnext(env->extras_after);
		}
					...
					switch (constraint->contype)
					{
						...
						case CONSTR_ATTR_IMMEDIATE:
                                                    /* transformConstraintAttrs took care of these */
                                                        break;
						case CONSTR_ATTR_STORAGE:
                                                        constraint->cooked_expr = pstrdup(column->colname);
							constraints = lappend(constraints, constraint);
							break;
 ...
 */
	...
	env->extras_before = blist;
	env->extras_after = ilist;
	...
	if (fkconstraints != NIL)
	{
		...
		foreach(fkclist, fkconstraints)
		{
			...
			env->extras_after = lappend(env->extras_after, (Node *) fk_trigger);
...
transformFkeyGetPrimaryKey(FkConstraint *fkconstraint)
{
	 ...
	 * ----------
	 */
	while (HeapTupleIsValid(indexTup = heap_getnext(indexSd)))
	{
...
GetAnalyzeInfo(void)
{
    AnalyzeGlobals* info = parseranalyze_globals;
    if ( info == NULL ) {
        info = AllocateEnvSpace(analyze_id,sizeof(AnalyzeGlobals));
		info->extras_before = NULL;
		info->extras_after = NULL;
        parseranalyze_globals = info;
    }
    return info;
}
