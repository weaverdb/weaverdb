 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/parser/parse_coerce.c,v 1.1.1.1 2006/08/12 00:21:09 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
			...
			Oid targetTypeId, int32 atttypmod)
{
	...
	else
	{
		...
		n->agg_distinct = false;
                ...
                cxt = targetType->t_datamcxt;
		result = transformExpr(pstate, (Node *) n, EXPR_COLUMN_FIRST);
                if ( targetType->t_datamcxt != cxt ) {
                    elog(ERROR,"bad context");
                }
