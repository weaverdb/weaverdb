 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/parser/parse_expr.c,v 1.1.1.1 2006/08/12 00:21:10 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <string.h>
...
typedef struct parse_expr_globals {
	int			expr_depth_counter;
} ParseExprGlobals;


static SectionId parse_id = SECTIONID("PEXR");

#ifdef TLS
TLS ParseExprGlobals* parse_expr_global = NULL;
#else
#define parse_expr_global GetEnv()->parse_expr_global
#endif

static ParseExprGlobals* GetParseExprInfo(void);
...
{
	GetParseExprInfo()->expr_depth_counter = 0;
}
...
transformExpr(ParseState *pstate, Node *expr, int precedence)
{
	 ...
	 */
	if (++GetParseExprInfo()->expr_depth_counter > max_expr_depth)
	...
	switch (nodeTag(expr))
	{
		...
		case T_ParamName:
			{
				ParamName    		*pna = (ParamName *) expr;
				char*			paramname = pna->name;
				Oid			toid = param_type_name(paramname); 
				Param	   		*param = makeNode(Param);
                                
                                if (!OidIsValid(toid))
					elog(ERROR, "Parameter '%s' is out of range", paramname);
				param->paramkind = PARAM_NAMED;
				param->paramid = (AttrNumber) -1;
				param->paramname = paramname;
				param->paramtype = toid;
				param->param_tlist = NIL;
				result = transformIndirection(pstate, (Node *) param,pna->indirection);
				/* cope with typecast applied to param */
				if (pna->typename != NULL)
					result = parser_typecast_expression(pstate, result,pna->typename);
				break;
			}
		case T_TypeCast:
			{
	...
	GetParseExprInfo()->expr_depth_counter--;
...
ParseExprGlobals*
GetParseExprInfo(void)
{
    ParseExprGlobals* info = parse_expr_global;

    if ( info == NULL ) {
        info = AllocateEnvSpace(parse_id,sizeof(ParseExprGlobals));
		info->expr_depth_counter = 0;
        parse_expr_global = info;
    }
