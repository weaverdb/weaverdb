 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/storage/smgr/md.c,v 1.1.1.1 2006/08/12 00:21:30 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
#include <errno.h>
...
/*  
	alloc and free are gated so that access is serialized to these 
	variables.  At some point it might be good to share these stuctures 
	among various connections
	
	    MKS   12.26.2000  
*/
static mutex_t 	mdgate;
...
mdinit()
{
	...
	MdCxt = (MemoryContext)AllocSetContextCreate(MemoryContextGetTopContext(),
												   "MdMemoryContext",
	...
	mutex_init(&mdgate,USYNC_THREAD,0);
