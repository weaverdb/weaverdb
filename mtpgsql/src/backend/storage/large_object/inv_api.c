 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/storage/large_object/inv_api.c,v 1.1.1.1 2006/08/12 00:21:27 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
...
inv_drop(Oid lobjId)
{
	...
	r = RelationIdGetRelation(lobjId,DEFAULTDBOID);
...
inv_read(LargeObjectDesc *obj_desc, char *buf, int nbytes)
{
	...
	while (nread < nbytes)
	{
		...
		d = HeapGetAttr(&tuple, 2, obj_desc->hdesc, &isNull);
...
inv_fetchtup(LargeObjectDesc *obj_desc, HeapTuple tuple, Buffer *buffer)
{
		...
		|| !ItemPointerIsValid(&(obj_desc->htid)))
	{
		...
		do
		{
			 ...
			 *
			 */
			tuple->t_self = obj_desc->iscan->xs_ctup.t_self;
	...
	d = HeapGetAttr(tuple, 1, obj_desc->hdesc, &isNull);
		  ...
		  Buffer buffer)
{
	...
	TransactionIdStore(xid, &(tuple->t_data->t_xmax));
	tuple->t_data->cmd.t_cmax = GetCurrentCommandId();
			 ...
			 int nwrite)
{
	...
	GetEnv()->LastOidProcessed = ntup->t_data->t_oid = newoid();
	xid = GetCurrentTransactionId();
	TransactionIdStore(xid, &(ntup->t_data->t_xmin));
	ntup->t_data->cmd.t_cmin = GetCurrentCommandId();
	StoreInvalidTransactionId(&(ntup->t_data->t_xmax));
	ntup->t_data->cmd.t_cmax = 0;
...
inv_indextup(LargeObjectDesc *obj_desc, HeapTuple tuple)
{
	...
	res = index_insert(obj_desc->index_r, &v[0], &n[0],
					   &(tuple->t_self), obj_desc->heap_r, false);
...
_inv_getsize(Relation hreln, TupleDesc hdesc, Relation ireln)
{
	...
	do
	{
		 ...
		 * time qual in.
		 */
		tuple.t_self = iscan->xs_ctup.t_self;
	...
	d = HeapGetAttr(&tuple, 1, hdesc, &isNull);
