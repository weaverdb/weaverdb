 ...
 *
 * IDENTIFICATION
 *	  $Header: /cvs/weaver/mtpgsql/src/backend/storage/ipc/shmem.c,v 1.1.1.1 2006/08/12 00:21:24 synmscott Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
InitShmem(unsigned int key, size_t size,int maxBackends)
{
	...
	/*  next is the total size of the memory segment  */
	if ( !size ) ShmemSize = *(unsigned long *)(ShmemFreeStart + 1);
	/* next is a shmem pointer to the shmem index */
	ShmemIndexOffset = ShmemFreeStart + 2;
	...
	currFreeSpace += sizeof(ShmemFreeStart) + sizeof(ShmemIndexOffset) +
		MAXALIGN(sizeof(VariableCacheData));
	...
	info.entrysize = SHMEM_INDEX_ENTRYSIZE;
